...
struct WorldItem
{
	...
	bool blue = false;
	int intdata = 0;
	/*Spotlight Real GT (Code by GTIR)*/
	string spotTo = "";
	/*VIP ENTRANCE*/
	vector<string> viplist;
	bool VipPublic = false;
	/*END V ENTRANCE*/
	/*GATEWAY TO ADVENTURE - CODED BY GTIR*/
	// World Destionations //
	string destGate1 = "";
	string destGate2 = "";
	string destGate3 = "";
	string destGate4 = "";
	// World Desinations ID //
	string destGateId1 = "";
	string destGateId2 = "";
	string destGateId3 = "";
	string destGateId4 = "";
	...
	try {
		...
		if (cave) {
			...
			for (auto i = 1; i < width; i++)
			{
				...
				if (rnd < -2)
					offset[i] = offset[i - 1] - 0; // rise
				else if (rnd > 11)
					offset[i] = offset[i - 1] + 0; // fall
				else
					offset[i] = offset[i - 1]; // no change
				offset2[i] = offset[i] < 0 ? -10 : offset[i];
			}
...
void GenerateBeachWorld(WorldInfo* world, int width, int height, int dirtType = 2, int lavaType = 4, int mainDoorType = 6, int bedrockType = 8, int rockType = 10, int caveBackgroundType = 14, int wmachineid = 0, int deco1Id = 0, int deco2Id = 0, int id = 0)
{
	world->weather = wmachineid;
...
inline int PlayerDB::guildRegister(ENetPeer* peer, string guildName, string guildStatement, string guildFlagfg, string guildFlagbg) {
	...
	if (fg > maxItems || bg > maxItems) return -1;
	auto guildNames = PlayerDB::getProperName(guildName);
	std::ifstream ifs("save/guilds/_" + guildNames + ".json");
	if (ifs.is_open()) {
		return -4;
	}
	return 1;
...
inline void removeAccount(string account) {
	std::remove(("save/players/_" + account + ".json").c_str());
}
...
void WorldDB::remove(string name) {
	try {
		...
		std::remove(("save/worlds/_" + name + ".json").c_str());
	}
		...
		try {
			...
			if (!read_world.is_open()) {
				if (usedtinyblast == true) {
					...
					ret.ptr = &worlds.at(worlds.size() - 1);
					return ret;
				} else if (usedbeachblast == true) {
					usedbeachblast = false;
					WorldInfo* info = worldDB.get_pointer(name);
					GenerateBeachWorld(info, 100, 60, 442, 442, 6, 8, 10, 850, 1, 848, 6324, BlastProperties_RandomDeco1);
					return ret;
				}
			...
			string worldGuild = j["worldGuild"];
			bool nuked = j["nuked"];
			...
			info.owner = owner;
			info.isNuked = nuked;
			info.worldGuild = worldGuild;
			info.isPublic = is_public;
			info.silence = silence;
			info.DisableDrop = disableDrop;
			info.rainbow = rainbow;
			info.disable_music = j["disable_music"];
			info.disable_music_render = j["disable_music_render"];
			info.tempo = j["tempo"];
			info.AllowNoClip = j["AllowNoClip"];
			if (admins != "") {
			...
			for (int i = 0; i < w_tiles.size(); i++) {
				...
				for (int Vip = 0; Vip < tiles.at(i)["VipAcc"].size(); Vip++) {
					w_tiles.viplist.push_back(tiles.at(i)["VipAcc"][Vip]);
				}
				w_tiles.VipPublic = tiles.at(i)["VipPublic"].get<bool>();
				w_tiles.gateLabel1 = tiles.at(i)["gtlab1"].get<string>();
				w_tiles.gateLabel2 = tiles.at(i)["gtlab2"].get<string>();
				w_tiles.gateLabel3 = tiles.at(i)["gtlab3"].get<string>();
				w_tiles.gateLabel4 = tiles.at(i)["gtlab4"].get<string>();
				w_tiles.gateBt1 = tiles.at(i)["gtbt1"].get<string>();
				w_tiles.gateBt2 = tiles.at(i)["gtbt2"].get<string>();
				w_tiles.gateBt3 = tiles.at(i)["gtbt3"].get<string>();
				w_tiles.gateBt4 = tiles.at(i)["gtbt4"].get<string>();
				w_tiles.destGate1 = tiles.at(i)["gtd1"].get<string>();
				w_tiles.destGate2 = tiles.at(i)["gtd2"].get<string>();
				w_tiles.destGate3 = tiles.at(i)["gtd3"].get<string>();
				w_tiles.destGate4 = tiles.at(i)["gtd4"].get<string>();
				w_tiles.destGateId1 = tiles.at(i)["gtdid1"].get<string>();
				w_tiles.destGateId2 = tiles.at(i)["gtdid2"].get<string>();
				w_tiles.destGateId3 = tiles.at(i)["gtdid3"].get<string>();
				w_tiles.destGateId4 = tiles.at(i)["gtdid4"].get<string>();
				w_tiles.display1 = tiles.at(i)["ds1"].get<int>();
				w_tiles.display2 = tiles.at(i)["ds2"].get<int>();
				w_tiles.display3 = tiles.at(i)["ds3"].get<int>();
				w_tiles.display4 = tiles.at(i)["ds4"].get<int>();
				...
				w_tiles.text = tiles.at(i)["text"].get<string>();
...
inline void updateGuild(ENetPeer* peer) {
	...
	if (guildname != "") {
		...
		if (ifff.fail()) {
			ifff.close();
			cout << "[!] Failed loading guilds/_" + guildname + ".json! From " + ((PlayerInfo*)(peer->data))->displayName + "." << endl;
...
inline void outFromGuild(ENetPeer* peer) {
	string guildname = PlayerDB::getProperName(((PlayerInfo*)(peer->data))->guild);
	if (guildname != "") {
		((PlayerInfo*)(peer->data))->guild = "";
	}
...
inline void updateSignSound(ENetPeer* peer, int foreground, int x, int y, string text, int background)
{
...
inline void sendConsole(ENetPeer* x, string e) {
...
inline void PlayAudio2(ENetPeer* peer, string audioFile, int delayMS) {
	const string text = "action|play_music\nfile|" + audioFile + "\ndelayMS|" + to_string(delayMS) + "\n";
	...
	while (updateAnswer)
	{
		...
		if (quest + 300000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
		{
				...
				++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
					continue;
				sendConsole(currentPeer, "`9** GTIR Daily Math : `3'" + to_string(resultnbr1) + " + " + to_string(resultnbr2) + "'`9 = ? Prize: `2" + to_string(prize) + "`9 (gems) ! `o(/a <answer>).");
			}
			cout << " ** GTIR Daily Math : '" + to_string(resultnbr1) + " + " + to_string(resultnbr2) + "' = ? Prize: " + to_string(prize) + " gems ! `o(/a answer)." << endl;
		}
	}
}

inline void DailyQuestReset() {
	while (true) {
		Sleep(21600000);
		remove("save/dailyquest/dailyitem1.txt");
		remove("save/dailyquest/dailyitem2.txt");
		remove("save/dailyquest/dailycount1.txt");
		remove("save/dailyquest/dailycount2.txt");
		remove("save/dailyquest/completed");
		ofstream createFolder("save/dailyquest/_completed/");
		createFolder.close();
		bool changed1 = std::experimental::filesystem::exists("save/dailyquest/dailyitem1.txt");
		if (changed1 == false)
		{
			int DailyItem1 = rand() % 3500;
			ofstream myfile;
			myfile.open("save/dailyquest/dailyitem1.txt");
			myfile << DailyItem1;
			myfile.close();
		}
		bool changed2 = std::experimental::filesystem::exists("save/dailyquest/dailyitem2.txt");
		if (changed2 == false)
		{
			int DailyItem2 = rand() % 3500;
			ofstream myfile;
			myfile.open("save/dailyquest/dailyitem2.txt");
			myfile << DailyItem2;
			myfile.close();
		}
		bool changed3 = std::experimental::filesystem::exists("save/dailyquest/dailycount1.txt");
		if (changed3 == false)
		{
			int DailyCount1 = rand() % 200;
			ofstream myfile;
			myfile.open("save/dailyquest/dailycount1.txt");
			myfile << DailyCount1;
			myfile.close();
		}
		bool changed4 = std::experimental::filesystem::exists("save/dailyquest/dailycount2.txt");
		if (changed4 == false)
		{
			int DailyCount2 = rand() % 200;
			ofstream myfile;
			myfile.open("save/dailyquest/dailycount2.txt");
			myfile << DailyCount2;
			myfile.close();
		}
...
inline void sendNothingHappened(ENetPeer* peer, int x, int y) {
	PlayerMoving data;
...
inline void setupVIP(BYTE* raw) {
	...
	raw[7] = 0x0; //0
	raw[8] = 0x0; //0
	raw[9] = 0x8000000; //134217728
	raw[10] = 0x80000; //524288
	raw[11] = 0x800; //2048
	raw[12] = 0x8; //8
	raw[13] = 0x0; //0
	raw[14] = 0x0; //0
	...
	raw[75] = 0x5800; //22528
	raw[76] = 0x58; //88
	raw[77] = 0x23000000; //587202560
	raw[78] = 0x230000; //2293760
}

inline void VIP(ENetPeer* peer, int x, int y, int fg, int bg, int owner) {
	int plength = 79;
...
BYTE* makeData(BlockData bdata) {
	...
	memcpy(data, &bdata.packetType, sizeof(short));
	memcpy(data + 12, &bdata.charState, sizeof(short));
	memcpy(data + 44, &bdata.x, sizeof(int));
	memcpy(data + 48, &bdata.y, sizeof(int));
	memcpy(data + 56, &bdata.fg, sizeof(short));
	memcpy(data + 58, &bdata.bg, sizeof(short));
	memcpy(data + 60, &bdata.visual, sizeof(int));
	memcpy(data + 64, &bdata.visual2, sizeof(int));
	return data;
}

inline void SendBlockData(ENetPeer* peer, BYTE* raw, int length, int length2) {
	ENetPacket* p = enet_packet_create(0, length, ENET_PACKET_FLAG_RELIABLE);
	int four = 4;
	memcpy(p->data, &four, sizeof(int));
	memcpy((char*)p->data + 4, raw, length2);
	enet_peer_send(peer, 0, p);
}
...
void UpdatePort(ENetPeer* peer, int x, int y, int fg, int bg, string data) {
	...
	for (auto& a : p) {
		if (a.length() == 0 || a == "") continue;
			...
			auto xd = explode("|", a);
			if (f1 == -1) { f1 = stoi(xd[0]); lb1 = stoi(xd[1]); }
			else if (f2 == -1) { f2 = stoi(xd[0]); lb2 = stoi(xd[1]); }
			else if (f3 == -1) { f3 = stoi(xd[0]); lb3 = stoi(xd[1]); }
			else if (f4 == -1) { f4 = stoi(xd[0]); lb4 = stoi(xd[1]); }
			else if (f5 == -1) { f5 = stoi(xd[0]); lb5 = stoi(xd[1]); }
		}
	}
...
void SendGeigerCharger(ENetPeer* peer, short fg, short bg, const int x, const int y, int item) {
...
inline void Portrait(ENetPeer* peer, int x, int y, int fg, int bg, int hairs, int mask, int face, int skin, int emoji, string text, int state) {
	...
	data.visual = state;// + getBlockState(world, x, y);
	...
	SendBlockData(peer, raw, plength + 5 + 4, plength); //made by KIPAS GTS
	delete[] raw;
}
...
inline void restoreplayernick(ENetPeer* peer)
{
	...
	try {
		if (static_cast<PlayerInfo*>(peer->data)->rawName == world->owner) {
		...
		} else if (static_cast<PlayerInfo*>(peer->data)->guild != "" && static_cast<PlayerInfo*>(peer->data)->rawName == static_cast<PlayerInfo*>(peer->data)->guildLeader && static_cast<PlayerInfo*>(peer->data)->displayName.find(" `e(GL)``") == string::npos) {
			static_cast<PlayerInfo*>(peer->data)->displayName += " `e(GL)``";
		}
		else if (static_cast<PlayerInfo*>(peer->data)->guild != "" && static_cast<PlayerInfo*>(peer->data)->rawName != static_cast<PlayerInfo*>(peer->data)->guildLeader) {
			...
			if (find(GE.begin(), GE.end(), static_cast<PlayerInfo*>(peer->data)->rawName) != GE.end() && static_cast<PlayerInfo*>(peer->data)->displayName.find(" `9(GE)``") == string::npos) {
				static_cast<PlayerInfo*>(peer->data)->displayName += " `9(GE)``";
			}
			else if (find(GC.begin(), GC.end(), static_cast<PlayerInfo*>(peer->data)->rawName) != GC.end() && static_cast<PlayerInfo*>(peer->data)->displayName.find(" `6(GC)``") == string::npos) {
				static_cast<PlayerInfo*>(peer->data)->displayName += " `6(GC)``";
			}
		}
	}
	catch (const std::out_of_range& e) {
...
void SendDisplayShelf(ENetPeer* peer, int itemid, int x, int y, int item1, int item2, int item3, int item4) {
	PlayerMoving pmov;
	pmov.packetType = 5;
	...
	*(int*)(packet + 4 + 56 + 9 + 3 * 4) = item4;
	ENetPacket* epacket = enet_packet_create(packet, 4 + 56 + 9 + 16, ENET_PACKET_FLAG_RELIABLE);
	enet_peer_send(peer, 0, epacket);
	delete pmovpacked;
	delete[] packet;
}
...
inline void sendPlayerLeave(ENetPeer* peer) {
	...
	static_cast<PlayerInfo*>(peer->data)->isShining = false;
	...
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		if (isHere(peer, currentPeer)) {
			...
			auto cctv = std::experimental::filesystem::exists("save/cctv/_" + world->name + ".json");
			for (int i = 0; i < world->width * world->height; i++) {
			...
			if (found_one) {
				ifstream ifff("save/cctv/_" + world->name + ".json");
				json j;
				ifff >> j;
				ifff.close();
				if (j["ShowPoepleExit"] == true && j["DontShowOwner"] == true && j["DontShowAdmin"] == true) {
					if (world->owner != static_cast<PlayerInfo*>(peer->data)->rawName || !isWorldAdmin(peer, world)) {
						...
						string tolog = "`w" + ((PlayerInfo*)(peer->data))->displayName + " `wleft at " + to_string(Hour) + ":" + to_string(Min) + " on " + to_string(Mon) + "/" + to_string(Day) + "";
						currentLogs.push_back(tolog);
						j["logs"] = currentLogs;
						ofstream fs("save/cctv/_" + world->name + ".json");
						fs << j;
						fs.close();
					}
				}
				else if (j["ShowPoepleExit"] == true && j["DontShowOwner"] == true) {
				...
				else if (j["ShowPoepleExit"] == true && j["DontShowAdmin"] == true) {
				...
				else if (j["ShowPoepleExit"] == false && j["DontShowOwner"] == false && j["DontShowAdmin"] == false || j["ShowPoepleExit"] == false && j["DontShowOwner"] == false || j["ShowPoepleExit"] == false && j["DontShowAdmin"] == false || j["ShowPoepleExit"] == false && j["DontShowAdmin"] == true || j["ShowPoepleExit"] == false && j["DontShowOwner"] == true || j["ShowPoepleExit"] == false && j["DontShowOwner"] == true && j["DontShowAdmin"] == true) {
				}
				else {
					vector<string> currentLogs;
					for (int i = 0; i < j["logs"].size(); i++) {
...
inline void sendRoulete(ENetPeer* peer)
{
		...
		++currentPeer)
	{
		...
		if (isHere(peer, currentPeer))
		{
			...
			else if (plrRole == "owner" || plrRole == "owner") {
				auto p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), static_cast<PlayerInfo*>(peer->data)->netID), "`w[" + name + " `wspun the wheel and got `20`w!]"), 0));
				auto respawnTimeout = 2000;
				auto deathFlag = 0x19;
				memcpy(p2.data + 24, &respawnTimeout, 4);
...
inline void doorlocked(ENetPeer* peer, int foreground, int x, int y, string text) {
	PlayerMoving sign;
	sign.packetType = 0x3;
	sign.characterState = 0x0;
	sign.x = x;
...
inline void checkMonitor(ENetPeer* peer, int x, int y) {
	bool found = false;
	int xx = 0;
	int yy = 0;
	...
	if (((PlayerInfo*)(peer->data))->currentWorld != "EXIT") {
		...
		if (world != nullptr) {
			for (int i = 0; i < world->width * world->height; i++) {
				if (world->items.at(i).foreground == 1240) {
					xx = (i % world->width);
					yy = (i / world->width);
					hmon_owner = world->items.at(i).monitorname;
					for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
						if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
						PlayerInfo* cp = ((PlayerInfo*)(currentPeer->data));
						if (cp->tankIDName == hmon_owner) {
							found = true;
						}
					}
					if (found) sendHMonitor(peer, xx, yy, hmon_owner, true, world->items.at(i).background);
					else sendHMonitor(peer, xx, yy, hmon_owner, false, world->items.at(i).background);
				}
			}
		}
	}
...
inline void UpdateDigiVend(ENetPeer* peer, int x, int y, int id, bool locks, int price, int background, bool isPer = false) {
	if (price == 0) id = 0;
	auto options321 = 0;
	if (price >= 256 && price < 512) options321 = 1;
	else if (price >= 512 && price < 768) options321 = 2;
...
inline void send_capcha_math()
{
	while (true) {
			...
			++currentPeer)
		{
			...
			if (static_cast<PlayerInfo*>(currentPeer->data)->level < 2) {
				Player::OnDialogRequest(currentPeer, captcha);
			}
		}
	}
}
...
int processTakeServers(ENetPeer* peer, const int itemIdx) {
...
void Dropitem2(ENetPeer* peer, int netID, int x, int y, int item, int count, BYTE specialEffect)
{
	WorldInfo* world = getPlyersWorld(peer);
	if (!world) return;
...
inline void sendWorldOffers(ENetPeer* peer) {
	try {
		...
		for (int i = 0; i < local_worlds.size(); i++) {
			...
			if (size > 0.75) size = 0.45;
			added_worlds++;
			worldOffers += "add_floater|" + local_worlds.at(i)->name + "|" + std::to_string(getPlayersCountInWorld(local_worlds.at(i)->name)) + "|0.5|3529161471\n";
		}
		...
		worldOffers += "add_heading|Recently Visited Worlds<CR>|";
...
inline void sendGuildInfo(ENetPeer* peer) {
	if (static_cast<PlayerInfo*>(peer->data)->guild == "") return;
	int guildLevel = 0;
	int guildXP = 0;
	int maxMemSize = 15;
	ifstream fg("save/guilds/_" + static_cast<PlayerInfo*>(peer->data)->guild + ".json");
	if (!fg.is_open()) {
		return;
	}
	json j;
	fg >> j;
	fg.close();
	guildLevel = j["GuildLevel"];
	guildXP = j["GuildExp"];
	if (guildLevel == 2) maxMemSize = 20;
	else if (guildLevel == 3) maxMemSize = 25;
	else if (guildLevel == 4) maxMemSize = 30;
	else if (guildLevel == 5) maxMemSize = 35;
	else if (guildLevel == 6) maxMemSize = 40;
	else if (guildLevel == 7) maxMemSize = 45;
	else if (guildLevel == 8) maxMemSize = 50;

	if (guildLevel == 8) {
		Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_player_info|" + static_cast<PlayerInfo*>(peer->data)->guild + "|" + std::to_string(guildLevel) + "|\nadd_spacer|small|\nadd_textbox|`oYour Guild " + static_cast<PlayerInfo*>(peer->data)->guild + " is Level (MAX!)|\nadd_spacer|small|\nadd_textbox|`oYou cannot upgrade your guild because your guild is already Level 5.|\nadd_smalltext|`oYour Guild Maximum Member's Size is : " + to_string(maxMemSize) + "|\nadd_spacer|small|\nadd_button|clsoeoe|Close|\nadd_quick_exit|");
	}
	else if (guildLevel < 8)
	{
		Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_player_info|" + static_cast<PlayerInfo*>(peer->data)->guild + "|" + std::to_string(guildLevel) + "|" + std::to_string(guildXP) + "|" + to_string(guildLevel * 3000) + "|\nadd_spacer|small|\nadd_textbox|`oYour Guild " + static_cast<PlayerInfo*>(peer->data)->guild + " is Level (" + to_string(guildLevel) + ")|\nadd_spacer|small|\nadd_textbox|`oYour guild will achieve XP's by farming, breaking, using geiger, getting fossils|\nadd_smalltext|All of your guild members will contribute guild XP if they did one of above.|\nadd_spacer|small|\nadd_textbox|Your guild will automatically upgraded if the Guild XP has reached the predetermined limit.|\nadd_smalltext|`oYour Guild Maximum Member's Size is : " + to_string(maxMemSize) + "|\nadd_spacer|small|\nadd_button|clsoeoe|Close|\nadd_quick_exit||");
	}
}

inline void sendGuildXP(ENetPeer* peer, int amount) {
	if (static_cast<PlayerInfo*>(peer->data)->guild == "") return;
	ifstream read_guild("save/guilds/_" + PlayerDB::getProperName(static_cast<PlayerInfo*>(peer->data)->guild) + ".json");
	...
	guildLevel = j["GuildLevel"];
	guildExp = j["GuildExp"];
	if (guildLevel == 8) return;
	if (guildExp >= (guildLevel * 3000) && guildLevel < 8) {
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
			if (((PlayerInfo*)(currentPeer->data))->guild == ((PlayerInfo*)(peer->data))->guild) {
				Player::OnConsoleMessage(currentPeer, "`9[GUILD ALERT] `oThe guild " + ((PlayerInfo*)(peer->data))->guild + " has reached Level " + to_string(guildLevel + 1) + "!");
				Player::OnAddNotification(currentPeer, "`wThe Guild " + ((PlayerInfo*)(peer->data))->guild + " has reached Level " + to_string(guildLevel + 1) + "!", "audio/gong.wav", "interface/large/gd_prom.rttex");
			}
		}
		try {
			auto gLevNow = guildLevel + 1;
			j["GuildLevel"] = gLevNow;
			j["GuildExp"] = 0;
	...
	try {
		auto bruh = guildExp + amount;
		j["GuildExp"] = bruh;
		ofstream write_guild("save/guilds/_" + PlayerDB::getProperName(static_cast<PlayerInfo*>(peer->data)->guild) + ".json");
...
inline void DropGem(ENetPeer* peer, const int x, const int y, WorldInfo* world, int kiek = 0) {
	try {
		...
		if (!world->drop_gem && b != 0) {
			try {
			    auto existGems = std::experimental::filesystem::exists("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
                if (!existGems) {

                    ofstream pGems("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
                    pGems << 0;
                    pGems.close();
                }
...
inline void SendCombiners(ENetPeer* peer, const int x, const int y, const int ItemID1, const int ItemID2, const int ItemID3, const int ItemID1Count, const int ItemID2Count, const int ItemID3Count, const int ResultItemID, const bool HarvestFestivalOnly, const int ResultItemCount = 1) {
	if (ItemID1 == 0 && ItemID2 == 0) {
		Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "`wI need 3 ingredients!", 0, false);
		return;
	}
					...
					if (world->droppedItems.at(pos1).id == ItemID2 && world->droppedItems.at(pos1).count == ItemID2Count) {
						for (auto pos = 0; pos < world->droppedItems.size(); pos++) {
							if (TwoIngredients) {
								...
								RemoveDroppedItem(peer, world->droppedItems.at(i).uid);
								RemoveDroppedItem(peer, world->droppedItems.at(pos1).uid);
								ItemBase2.push_back(world->droppedItems.at(i).uid);
								ItemBase.push_back(world->droppedItems.at(pos1).uid);
								for (auto ClearItem = 0; ClearItem < ItemBase.size(); ClearItem++) processTakeServers(peer, ItemBase.at(ClearItem));
								for (auto ClearItem2 = 0; ClearItem2 < ItemBase2.size(); ClearItem2++) processTakeServers(peer, ItemBase2.at(ClearItem2));
								DropItem(world, peer, -1, x * 32 + (rand() % 16), y * 32 + (rand() % 16), ResultItemID, ResultItemCount, 0);
								Player::OnTalkBubble(peer, ((PlayerInfo*)(peer->data))->netID, "SCIENCE!", 0, true);
								break;
							}
							else {
								if (world->droppedItems.at(pos).id == ItemID3 && world->droppedItems.at(pos).count == ItemID3Count) {
									...
									RemoveDroppedItem(peer, world->droppedItems.at(i).uid);
									RemoveDroppedItem(peer, world->droppedItems.at(pos1).uid);
									RemoveDroppedItem(peer, world->droppedItems.at(pos).uid);
									ItemBase2.push_back(world->droppedItems.at(i).uid);
									ItemBase.push_back(world->droppedItems.at(pos1).uid);
									ItemBase3.push_back(world->droppedItems.at(pos).uid);
									for (auto ClearItem = 0; ClearItem < ItemBase.size(); ClearItem++) processTakeServers(peer, ItemBase.at(ClearItem));
									for (auto ClearItem2 = 0; ClearItem2 < ItemBase2.size(); ClearItem2++) processTakeServers(peer, ItemBase2.at(ClearItem2));
									for (auto ClearItem3 = 0; ClearItem3 < ItemBase3.size(); ClearItem3++) processTakeServers(peer, ItemBase3.at(ClearItem3));
									DropItem(world, peer, -1, x * 32 + (rand() % 16), y * 32 + (rand() % 16), ResultItemID, ResultItemCount, 0);
									Player::OnTalkBubble(peer, ((PlayerInfo*)(peer->data))->netID, "SCIENCE!", 0, true);
									break;
								}
...
inline void send_item1(ENetPeer* peer, const int itemid, const int item_amount, const int kindofchest)
{
	auto success = false;
	SaveItemMoreTimes(itemid, item_amount, peer, success, static_cast<PlayerInfo*>(peer->data)->rawName + " from " + getItemDef(kindofchest).name + "");
	ENetPeer* currentPeer;
		...
		++currentPeer)
	{
		...
		if (isHere(peer, currentPeer))
		{
			GamePacket p1 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#[A `9Spirit Storage Unit`# exploded, bringing forth an `9" + getItemDef(itemid).name + "`# from The Other Side!]"));
...
inline void SendDigiVendDialog(ENetPeer* peer, WorldInfo* world) {
	try {
		const auto x = static_cast<PlayerInfo*>(peer->data)->lastPunchX;
		const auto y = static_cast<PlayerInfo*>(peer->data)->lastPunchY;
		const auto squaresign = x + (y * world->width);
		...
		auto currentworld = static_cast<PlayerInfo*>(peer->data)->currentWorld + "X" + std::to_string(squaresign);
		string withdrawdialog = "";
		if (world->items.at(x + (y * world->width)).vdraw > 0) withdrawdialog = "\nadd_smalltext|`oYou have earned " + to_string(world->items.at(x + (y * world->width)).vdraw) + " World Locks.``|left|\nadd_button|withdraw2|Withdraw World Locks|";
		if (world->items.at(x + (y * world->width)).vid == 0 || world->items.at(x + (y * world->width)).vcount == 0) {
			Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_textbox|This machine is empty.|left|\nadd_item_picker|stockitem1|`wPut an item in``|Choose an item to put in the machine!|" + withdrawdialog + "|\nend_dialog|digivending|Close||");
		}
		else if (world->items.at(x + (y * world->width)).vcount != 0) {
			string addthemdialog = "";
			auto mtitems = 0;
			...
			Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_label_with_icon|sml|The machine contains a total of " + to_string(world->items.at(x + (y * world->width)).vcount) + " `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``.|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|" + saledialog + "|" + addthemdialog + "|\nadd_button|pullstock2|Empty the machine|noflags|0|0|\nadd_smalltext|`5(DigiVend Machine will not function when price is set to 0)``|left|\nadd_text_input|setprice|Price|" + to_string(world->items.at(x + (y * world->width)).vprice) + "|5|" + type + "|" + withdrawdialog + "|\nend_dialog|digivending|Close|Update|");
		}
		else if (world->items.at(x + (y * world->width)).vcount != 0 && world->items.at(x + (y * world->width)).vprice != 0) {
			if (world->items.at(x + (y * world->width)).opened) Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_label_with_icon|sml|The machine contains a total of " + to_string(world->items.at(x + (y * world->width)).vcount) + " `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``.|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_textbox|For a cost of:|left|\nadd_label_with_icon|small|1 x `8World Lock``|left|242|\nadd_spacer|small|\nadd_textbox|You will get:|left|\nadd_label_with_icon|small|" + to_string(world->items.at(x + (y * world->width)).vprice) + " x `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_button|pullstock2|Empty the machine|noflags|0|0|\nadd_smalltext|`5(DigiVend Machine will not function when price is set to 0)``|left|\nadd_text_input|setprice|Price|" + to_string(world->items.at(x + (y * world->width)).vprice) + "|5|" + type + "|" + withdrawdialog + "|\nend_dialog|digivending|Close|Update|");
			else Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_label_with_icon|sml|The machine contains a total of " + to_string(world->items.at(x + (y * world->width)).vcount) + " `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``.|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_textbox|For a cost of:|left|\nadd_label_with_icon|small|" + to_string(world->items.at(x + (y * world->width)).vprice) + " x `8World Lock``|left|242|\nadd_spacer|small|\nadd_textbox|You will get:|left|\nadd_label_with_icon|small|1 x `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_button|pullstock2|Empty the machine|noflags|0|0|\nadd_smalltext|`5(DigiVend Machine will not function when price is set to 0)``|left|\nadd_text_input|setprice|Price|" + to_string(world->items.at(x + (y * world->width)).vprice) + "|5|" + type + "|" + withdrawdialog + "|\nend_dialog|digivending|Close|Update|");
...
inline void SendBuyerDigiVendDialog(ENetPeer* peer, WorldInfo* world) {
	try {
		const int x = static_cast<PlayerInfo*>(peer->data)->lastPunchX;
		const int y = static_cast<PlayerInfo*>(peer->data)->lastPunchY;
		const auto squaresign = x + (y * world->width);
		...
		if (world->items.at(x + (y * world->width)).vid == 0 || world->items.at(x + (y * world->width)).vcount == 0 || world->items.at(x + (y * world->width)).vprice == 0 || world->items.at(x + (y * world->width)).vcount < world->items.at(x + (y * world->width)).vprice && world->items.at(x + (y * world->width)).opened) {
			...
			locks += worldlocks;
			if (world->items.at(x + (y * world->width)).opened) Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_label_with_icon|sml|The machine contains a total of " + to_string(world->items.at(x + (y * world->width)).vcount) + " `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``.|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_textbox|For a cost of:|left|\nadd_label_with_icon|small|1 x `8World Lock``|left|242|\nadd_spacer|small|\nadd_textbox|You will get:|left|\nadd_label_with_icon|small|" + to_string(world->items.at(x + (y * world->width)).vprice) + " x `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_textbox|You have " + to_string(locks) + " World Locks.|left|\nadd_text_input|buycount|How many would you like to buy?|0|3|\nend_dialog|digivending|Close|Buy|");
			else Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_label_with_icon|sml|The machine contains a total of " + to_string(world->items.at(x + (y * world->width)).vcount) + " `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``.|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_textbox|For a cost of:|left|\nadd_label_with_icon|small|" + to_string(world->items.at(x + (y * world->width)).vprice) + " x `8World Lock``|left|242|\nadd_spacer|small|\nadd_textbox|You will get:|left|\nadd_label_with_icon|small|1 x `2" + getItemDef(world->items.at(x + (y * world->width)).vid).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).vid) + "|\nadd_spacer|small|\nadd_textbox|You have " + to_string(locks) + " World Locks.|left|\nadd_text_input|buycount|How many would you like to buy?|0|3|\nend_dialog|digivending|Close|Buy|");
		}
	}
	catch (const std::out_of_range& e) {
	...
	equip:
		...
		for (int i = 0; i < x; i++) {
			...
			switch (id) {
				...
				case 11232:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 328672;
					break;
				}
				case 1946:
				{
				...
				case 8002:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 114;
					break;
				}
				case 1874:
				{
				...
				case 11118:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 2010;
					break;
				}
				case 2720:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 271660;
					break;
				}
				case 2386:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 271652;
					break;
				}
				case 3124:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 271674;
					break;
				}
				case 4644:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 271689;
					break;
				}
				case 7098:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 271706;
					break;
				}
				case 7488:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 271719;
					break;
				}
				case 1780:
				{
				...
				case 6804:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 106;
					break;
				}
				case 2866:
				{
				...
				case 4464:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 73;
					break;
				}
				case 6312:
				{
				...
				case 10388:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 180;
					break;
				}
				case 2212:
				{
				...
				case 9116: case 9118: case 9120: case 9122:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 8424243 + 111;
					break;
				}
				case 2872:
				{
				...
				case 2952: case 9510: case 9512: case 1438: case 9520: case 9514: case 9496: case 9492: case 9494: case 9488: case 3932: case 9448: case 9452: case 9508: case 9454: case 9456: case 9458: case 9430:
				{
				...
				case 7126:
					static_cast<PlayerInfo*>(peer->data)->effect = 8421470;
					break;
				case 7650:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 8421481;
					break;
				}
				case 7586:
				{
				...
				default:
				{
					if (type == 6) {
						...
						if (id == 4996) {
							Player::OnTextOverlay(peer, "`4This item if banned from GTIR!");
							static_cast<PlayerInfo*>(peer->data)->effect = 8421376;
							break;
						}
						else if (id == 3774)
						{
							...
							send_state(peer);
							if (static_cast<PlayerInfo*>(peer->data)->cloth_back == 10426 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 156 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9006 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1958 || getItemDef(static_cast<PlayerInfo*>(peer->data)->cloth_back).name.find("Aura") != string::npos || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9476 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9434 || getItemDef(static_cast<PlayerInfo*>(peer->data)->cloth_back).name.find("Wings") != string::npos || getItemDef(static_cast<PlayerInfo*>(peer->data)->cloth_back).name.find("Cape") != string::npos || static_cast<PlayerInfo*>(peer->data)->cloth_back == 8580 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 5158 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9410 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9306 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 10022 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 8308 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 7834 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 10020 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9760 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 10016 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 10012 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1674 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9478 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9506 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9476 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1970 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 8286 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9506 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9478 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 8552 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 362 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 678 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 736 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 7734 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 7762 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 818 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1206 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1460 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1550 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1574 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1668 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1672 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1674 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1784 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1824 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1936 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1938 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1970 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2254 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2256 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2258 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2260 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2262 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2264 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2390 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2392 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3120 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3308 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3512 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4534 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4986 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 5754 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6144 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6334 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6694 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6818 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6842 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1934 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3134 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6004 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1780 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2158 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2160 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2162 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2164 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2166 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2168 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2438 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2538 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2778 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3858 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 350 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 998 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 1738 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2642 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2982 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3104 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3144 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 5738 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3112 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 2722 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3114 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4970 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4972 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 5020 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 6284 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4184 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4628 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 5322 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4112 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 4114 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 3442 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9466 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 8286 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 5136 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9416 || static_cast<PlayerInfo*>(peer->data)->cloth_back == 9356) {
								static_cast<PlayerInfo*>(peer->data)->canDoubleJump = true;
							}
							else {
								static_cast<PlayerInfo*>(peer->data)->canDoubleJump = false;
						...
						if (((PlayerInfo*)(peer->data))->noEyes = true && id != 3774) {
...
inline void restore_player_name(WorldInfo* world, ENetPeer* peer) {
	try {
		...
		if (static_cast<PlayerInfo*>(peer->data)->ltitle && static_cast<PlayerInfo*>(peer->data)->ltitleunlocked && name2.find(" of Legend``") == string::npos) {
		...
		else if (static_cast<PlayerInfo*>(peer->data)->guild != "" && static_cast<PlayerInfo*>(peer->data)->rawName == static_cast<PlayerInfo*>(peer->data)->guildLeader && name2.find(" `e(GL)``") == string::npos) {
			name2 += " `e(GL)``";
		}
		else if (static_cast<PlayerInfo*>(peer->data)->guild != "" && static_cast<PlayerInfo*>(peer->data)->rawName != static_cast<PlayerInfo*>(peer->data)->guildLeader) {
			std::ifstream ifff("save/guilds/_" + PlayerDB::getProperName(static_cast<PlayerInfo*>(peer->data)->guild) + ".json");
			...
			if (find(GE.begin(), GE.end(), static_cast<PlayerInfo*>(peer->data)->rawName) != GE.end() && name2.find(" `9(GE)``") == string::npos) {
				name2 += " `9(GE)``";
			}
			else if (find(GC.begin(), GC.end(), static_cast<PlayerInfo*>(peer->data)->rawName) != GC.end() && name2.find(" `6(GC)``") == string::npos) {
				name2 += " `6(GC)``";
			}
		}
...
string get_player_buffss(PlayerInfo* pData, WorldInfo* world) {
	string buffs = "";
	if (isPickaxe(pData))
	{
	...
	if (pData->cloth_hand == 5480) {
		buffs += "|\nadd_label_with_icon|small|`wRayman's Fist``|left|" + to_string(pData->cloth_hand) + "|";
	}
	if (pData->cloth_hand == 2952)
	{
		buffs += "|\nadd_label_with_icon|small|`wDigger's Spade``|left|" + to_string(pData->cloth_hand) + "|";
	}
...
inline void LoadPlayerData(ENetPeer* peer) {
	...
	if (pData->haveGrowId && pData->rawName != "") {
		try {
			...
			string email = j["email"];
			int playerid = j["playerid"];
			int billboardid = j["billboardid"];
			int billboardtab = j["billboardtab"];
			int billboardprice = j["billboardprice"];
			int billboardperwhat = j["billboardperwhat"];
			...
			if (billboardperwhat == 0 || billboardid == 0 || billboardprice == 0 || billboardtab == 0) {

			}
			else {
				BillBoard(peer, billboardid, billboardprice, billboardtab, billboardperwhat);
			}
			...
			pData->billboardid = billboardid;
			pData->billboardperwhat = billboardperwhat;
			pData->billboardprice = billboardprice;
			pData->billboardtab = billboardtab;
			string guildname = PlayerDB::getProperName(((PlayerInfo*)(peer->data))->guild);
			if (guildname != "") {
				std::ifstream ifff("save/guilds/_" + guildname + ".json");
				if (ifff.fail()) {
...
inline void send_quest_view_bot(ENetPeer* peer, PlayerInfo* pData, WorldInfo* world) {
	if (!pData->quest_active) return;
	try {
		int x = pData->lastPunchX;
		int y = pData->lastPunchY;
		...
		if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) || world->items.at(x + (y * world->width)).foreground == 1790) {
			if (pData->lastquest == "steel") {
				...
				case 1:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 2000) KiekTuri = 2000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Chemical G";
					}
					break;
				}
				case 2:
				...
				case 4:
				{
					quest_info = "I challenge you to bring me 600 of those Robot Wants Dubsteps thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/600)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 600) KiekTuri = 600 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Robot Wants Dubstep";
					}
					break;
				}
				case 5:
				...
				case 7:
				{
					quest_info = "I challenge you to bring me 3 of those Edison Zoomer thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/3)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Edison Zoomer";
					}
					break;
				}
				case 8:
				...
				case 9:
				{
					quest_info = "I challenge you to bring me 1000 of those High Tech Block thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1000)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						...
						deliver = "Deliver " + to_string(KiekTuri) + " Higt Tech Block";
					}
					break;
				}
				case 10:
				...
				case 14:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Bride Of Reanimator Remote";
					}
					break;
				}
				case 15:
				...
				case 16:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Mint Julep";
					}
					break;
				}
				case 17:
				{
					...
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/5)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						...
						deliver = "Deliver " + to_string(KiekTuri) + " Kerjigger";
					}
					break;
				}
				case 18:
				{
					quest_info = "I challenge you to bring me 5 of those Doohickeys thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/5)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 5) KiekTuri = 5 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Doohickey";
					}
					break;
				}
				case 19:
				{
					quest_info = "I challenge you to bring me 2 of those Thingamabob thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/2)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						...
						deliver = "Deliver " + to_string(KiekTuri) + " Thingamabob";
					}
					break;
				}
				case 20:
...
inline void send_quest_view_dragon(ENetPeer* peer, PlayerInfo* pData, WorldInfo* world) {
	if (!pData->quest_active) return;
	try {
		int x = pData->lastPunchX;
		int y = pData->lastPunchY;
		...
		if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) || world->items.at(x + (y * world->width)).foreground == 1790) {
			if (pData->lastquest == "fire") {
				...
				case 1:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 2000) KiekTuri = 2000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Lava";
					}
					break;
				}
				case 2:
				...
				case 4:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 600) KiekTuri = 600 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Dragon Gate";
					}
					break;
				}
				case 5:
				...
				case 7:
				{
					...
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/10)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						...
						deliver = "Deliver " + to_string(KiekTuri) + " Dragon Hand";
					}
					break;
				}
				case 8:
				...
				case 9:
				{
					quest_info = "I challenge you to bring me 1000 of those Dragon Tail thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1000)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1000) KiekTuri = 1000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Dragon Tail";
					}
					break;
				}
				case 10:
				...
				case 14:
				{
					quest_info = "I challenge you to bring me 3 of those Fiesta Dragon thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/3)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Fiesta Dragon";
					}
					break;
				}
				case 15:
				...
				case 16:
				{
					quest_info = "I challenge you to bring me 1 of those WOTD Ttophy - 2021 thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " WOTD Trophy - 2021";
					}
					break;
				}
				case 17:
				{
					quest_info = "I challenge you to bring me 3 of those Ice Dragon Hand thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/3)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Ice Dragon Hand";
					}
					break;
				}
				case 18:
				{
					quest_info = "I challenge you to bring me 1000 of those Rocket Thruster thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1,000)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1000) KiekTuri = 1000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Rocket Thruster";
					}
					break;
				}
				case 19:
				{
					quest_info = "I challenge you to bring me 3 of those Devil Wings thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/3)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Devil Wings";
					}
					break;
				}
				case 20:
...
inline void send_quest_view_lwing(ENetPeer* peer, PlayerInfo* pData, WorldInfo* world) {
	if (!pData->quest_active) return;
	try {
		int x = pData->lastPunchX;
		int y = pData->lastPunchY;
		...
		if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) || world->items.at(x + (y * world->width)).foreground == 1790) {
			if (pData->lastquest == "heavens") {
				...
				case 1:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1000) KiekTuri = 1000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Clouds";
					}
					break;
				}
				case 2:
				...
				case 4:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 600) KiekTuri = 600 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Fairy Wings";
					}
					break;
				}
				case 5:
				...
				case 7:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Bubble Wings";
					}
					break;
				}
				case 8:
				...
				case 9:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 800) KiekTuri = 800 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Crimson Eagle Wings";
					}
					break;
				}
				case 10:
				...
				case 14:
				{
					quest_info = "I challenge you to bring me 20 of those Rainbow Wings thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/20)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
						if (pData->inventory.items.at(i).itemID == 1672 && pData->inventory.items.at(i).itemCount > 0) {
				...
				case 16:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Golden Angel Wings";
					}
					break;
				}
				case 17:
				{
					...
					if (KiekTuri != 0) {
						...
						deliver = "Deliver " + to_string(KiekTuri) + " Ripper Wings";
					}
					break;
				}
				case 18:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Phoenix Wings";
					}
					break;
				}
				case 19:
				{
					quest_info = "I challenge you to bring me 50 of those Parrot Wings thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/50)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						...
						deliver = "Deliver " + to_string(KiekTuri) + " Parrow Wings";
					}
					break;
				}
				case 20:
...
inline void send_quest_view_katana(ENetPeer* peer, PlayerInfo* pData, WorldInfo* world) {
	if (!pData->quest_active) return;
	try {
		int x = pData->lastPunchX;
		int y = pData->lastPunchY;
		...
		if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) || world->items.at(x + (y * world->width)).foreground == 1790) {
			if (pData->lastquest == "blade") {
				...
				case 1:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1000) KiekTuri = 1000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Iron Bars";
					}
					break;
				}
				case 2:
				...
				case 4:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 600) KiekTuri = 600 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Golden Sword";
					}
					break;
				}
				case 5:
				...
				case 7:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Heavenly Scythe";
					}
					break;
				}
				case 8:
				...
				case 9:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 800) KiekTuri = 800 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Headsman's Axe";
					}
					break;
				}
				case 10:
				...
				case 14:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 20) KiekTuri = 20 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Flamesaber";
					}
					break;
				}
				case 15:
				...
				case 16:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Golden Apple";
					}
					break;
				}
				case 17:
				{
					quest_info = "I challenge you to bring me 10 of those Much-Too-Small Yellow Shirt thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/10)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 10) KiekTuri = 10 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Much-Too-Small Yellow Shirt";
					}
					break;
				}
				case 18:
				{
					quest_info = "I challenge you to bring me 20 of those Flame Scythes thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/20)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 20) KiekTuri = 20 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Flame Scythe";
					}
					break;
				}
				case 19:
				{
					quest_info = "I challenge you to bring me 20 of those Crystal Glaive thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/20)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 20) KiekTuri = 20 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Crystal Glaive";
					}
					break;
				}
				case 20:
...
inline void send_quest_view_whip(ENetPeer* peer, PlayerInfo* pData, WorldInfo* world) {
	if (!pData->quest_active) return;
	try {
		int x = pData->lastPunchX;
		int y = pData->lastPunchY;
		...
		if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) || world->items.at(x + (y * world->width)).foreground == 1790) {
			if (pData->lastquest == "candour") {
				...
				case 1:
				{
					quest_info = "I challenge you to bring me 2,000 of those Secret of Growtopia thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/2,000)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 2000) KiekTuri = 2000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + "Secret of Growtopia";
					}
					break;
				}
				case 2:
				...
				case 4:
				{
					quest_info = "I challenge you to bring me 1000 of those Mind-Ghost-In-A-Jar thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1,000)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1000) KiekTuri = 1000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Mind-Ghost-In-A-Jar";
					}
					break;
				}
				case 5:
				...
				case 7:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 10) KiekTuri = 10 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Super Squirt Gun Jetpack";
					}
					break;
				}
				case 8:
				...
				case 9:
				{
					quest_info = "I challenge you to bring me 5 of those Soul Stone thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/5)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 5) KiekTuri = 5 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Soul Stone";
					}
					break;
				}
				case 10:
				...
				case 12:
				{
					quest_info = "Do some good for the world. Defeat 100 villains!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/100)";
					deliver = "You haven't helped enough!";
					if (pData->quest_progress >= 0) {
						deliver = "Defending people is what I do";
					}
					break;
				}
				case 13:
				...
				case 14:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Celestial Lance";
					}
					break;
				}
				case 15:
				...
				case 16:
				{
					quest_info = "I challenge you to bring me 5 of those Ring Of Shrinking thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/5)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 5) KiekTuri = 5 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Ring Of Shrinking";
					}
					break;
				}
				case 17:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Golden Talaria";
					}
					break;
				}
				case 18:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Ancestral Totems of Wisdom";
					}
					break;
				}
				case 19:
				{
					quest_info = "I challenge you to bring me 1 of those Summer Event Player Medal: Gold thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
						if (pData->inventory.items.at(i).itemID == 6094 && pData->inventory.items.at(i).itemCount > 0) {
							KiekTuri = pData->inventory.items.at(i).itemCount;
							break;
						}
					}
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Summer Event Player Medal: Gold";
					}
					break;
				}
				case 20:
...
inline void send_quest_view_lkwing(ENetPeer* peer, PlayerInfo* pData, WorldInfo* world) {
	if (!pData->quest_active) return;
	try {
		int x = pData->lastPunchX;
		int y = pData->lastPunchY;
		...
		if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) || world->items.at(x + (y * world->width)).foreground == 1790) {
			if (pData->lastquest == "thesky") {
				...
				case 1:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 2000) KiekTuri = 2000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Obsidian";
					}
					break;
				}
				case 2:
				...
				case 4:
				{
					quest_info = "I challenge you to bring me 1000 of those Knight Helmet thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/1,000)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1000) KiekTuri = 1000 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Knight Helmet";
					}
					break;
				}
				case 5:
				...
				case 7:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 10) KiekTuri = 10 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Blanket Cape";
					}
					break;
				}
				case 8:
				...
				case 9:
				{
					quest_info = "I challenge you to bring me 800 of those Blazing Electro Wings thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/800)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 800) KiekTuri = 800 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Blazing Electro Wings";
					}
					break;
				}
				case 10:
				...
				case 14:
				{
					quest_info = "I challenge you to bring me 10 of those Autumn Wings thingies!";
					current_progress = "(Current progress " + to_string(pData->quest_progress) + "/10)";
					deliver = "You have none to deliver!";
					auto KiekTuri = 0;
					for (auto i = 0; i < pData->inventory.items.size(); i++) {
						if (pData->inventory.items.at(i).itemID == 14824 && pData->inventory.items.at(i).itemCount > 0) {
							KiekTuri = pData->inventory.items.at(i).itemCount;
							break;
						}
					}
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 10) KiekTuri = 10 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Autumn Wings";
					}
					break;
				}
				case 15:
				...
				case 16:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Golden Dragon Statue";
					}
					break;
				}
				case 17:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 5) KiekTuri = 5 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Chaos Dragon";
					}
					break;
				}
				case 18:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 1) KiekTuri = 1 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Draconic Wings";
					}
					break;
				}
				case 19:
				{
					...
					if (KiekTuri != 0) {
						if (pData->quest_progress + KiekTuri > 10) KiekTuri = 10 - pData->quest_progress;
						deliver = "Deliver " + to_string(KiekTuri) + " Dragon Knight's Chestplate";
					}
					break;
				}
				case 20:
...
inline void send_info(ENetPeer* peer, PlayerInfo* pData) {
	...
	if (pData->cloth_necklace == 10246) {
		dragon_charm += "\nadd_custom_button|cdc|image:interface/large/gui_wrench_daylight_dragon.rttex;image_size_x:400;image_size_y:260;width: 0.19;|";
	}
	if (pData->cloth_face == 11508) {
	...
	if (pData->cloth_face == 11506) {
		dragonpass += "\nadd_custom_button|dragonpass|image:interface/large/gui_wrench_mask_dragon_transform.rttex;image_size_x:400;image_size_y:260;width: 0.19;";
	}
	if (pData->cloth_hand == 10960) {
	...
	if (pData->cloth_hand == 10952) {
		space_alien += "\nadd_custom_button|transform_alien|image:interface/large/gui_wrench_space_animals_transform.rttex;image_size_x:400;image_size_y:260;width: 0.19;";
	}
	if (pData->cloth_hand == 10954) {
		space_alien += "\nadd_custom_button|transform_alien|image:interface/large/gui_wrench_space_animals_transform.rttex;image_size_x:400;image_size_y:260;width: 0.19;";
	}
	if (pData->cloth_hand == 10956) {
		space_alien += "\nadd_custom_button|transform_alien|image:interface/large/gui_wrench_space_animals_transform.rttex;image_size_x:400;image_size_y:260;width: 0.19;";
	}
	if (pData->cloth_hand == 10958) {
		space_alien += "\nadd_custom_button|transform_alien|image:interface/large/gui_wrench_space_animals_transform.rttex;image_size_x:400;image_size_y:260;width: 0.19;";
	}
	if (static_cast<PlayerInfo*>(peer->data)->Busy == true) {
		status += "\nadd_custom_button|set_online_status|image:interface/large/gui_wrench_online_status_3red.rttex;image_size_x:400;image_size_y:260;width: 0.19;|";
	}
	else if (static_cast<PlayerInfo*>(peer->data)->Away == true)
	{
		status += "\nadd_custom_button|set_online_status|image:interface/large/gui_wrench_online_status_2yellow.rttex;image_size_x:400;image_size_y:260;width: 0.19;|";
	}
	else if (static_cast<PlayerInfo*>(peer->data)->Online == true)
	{
		status += "\nadd_custom_button|set_online_status|image:interface/large/gui_wrench_online_status_1green.rttex;image_size_x:400;image_size_y:260;width: 0.19;|";
	}
	else {
		status += "\nadd_custom_button|set_online_status|image:interface/large/gui_wrench_online_status_1green.rttex;image_size_x:400;image_size_y:260;width: 0.19;|";
	}
	if (pData->firesOutput > 0) Fire = "\nadd_smalltext|`oFires Put Out: " + to_string(((PlayerInfo*)(peer->data))->firesOutput) + "|\nadd_spacer|small|";
	int playtimehours = 0;
	ifstream fd("save/playtime/_" + ((PlayerInfo*)(peer->data))->rawName + ".txt");
	if (fd.is_open()) {
		fd >> playtimehours;
		fd.close();
	}
	...
	if (dq == true)
	{
		dailyquest += "\nadd_label_with_icon|small|`2Completed! `oWait 6 Hour for the new Daily Quest|left|1486|";
	}
	else
	{
		std::ifstream ifsz("save/dailyquest/dailyitem1.txt");
		...
		std::string contents2((std::istreambuf_iterator<char>(ifszs2)),
			(std::istreambuf_iterator<char>()));
		int quantity2 = atoi(contents2.c_str());
		dailyquest += "\nadd_label_with_icon|small|`oToday's Daily Quest: `w(" + to_string(quantity) + ") " + getItemDef(id).name + "`o and `w(" + to_string(quantity2) + ") " + getItemDef(id2).name + "`o.|left|1486|";
	}
	//\nadd_label_with_icon|small|`oToday's Daily Quest is: " + to_string(quantity) + " `o" + getItemDef(id).name + "`oand " + to_string(quantity2) + " `o" + getItemDef(id2).name + "``o You can deliver them via calling '12345' in telephone!|left|1486|
	Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_player_info|" + pData->displayName + "|" + std::to_string(levels) + "|" + std::to_string(xp) + "|" + to_string(pData->level * 1500) + "|\nadd_spacer|small|" + dailyquest + "|" + "\nadd_spacer|small|" + joinguilddialog + "|\nadd_button|change_password|`$Change password``|\nadd_spacer|small|\nset_custom_spacing|x:5;y:10|\nadd_custom_button|manage_title|image:interface/large/gui_wrench_title.rttex;image_size_x:400;image_size_y:260;width: 0.19;|" + status + "|\nadd_custom_button|billboardchange|image:interface/large/gui_wrench_edit_billboard.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_button|notebook_edit|image:interface/large/gui_wrench_notebook.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_button|goals|image:interface/large/gui_wrench_goals_quests.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_button|bonus_inactive|image:interface/large/gui_wrench_daily_bonus_inactive.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_button|worlds_view|image:interface/large/gui_wrench_my_worlds.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_button|achievements|image:interface/large/gui_wrench_achievements.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_button|emojis|image:interface/large/gui_wrench_growmojis.rttex;image_size_x:400;image_size_y:260;width: 0.19;" + "|" + rift_cape + "|" + space_alien + "|" + dragon_charm + "|" + dragonpass + "|\nadd_custom_break|\nadd_spacer|small|\nset_custom_spacing|x:0;y:0|\nadd_textbox|`oYou have `w" + to_string(static_cast<PlayerInfo*>(peer->data)->currentInventorySize) + "`o Backpack Slots.``|left|\nadd_textbox|`wActive effects:``|left|" + buffs + "|\nadd_spacer|small|" + Fire + "|\nadd_textbox|`oCurrent world: `w" + currentworld + " `o(`w" + std::to_string(xxx) + "`o, `w" + std::to_string(yyy) + "`o)  (`w" + std::to_string(getPlayersCountInWorld(((PlayerInfo*)(peer->data))->currentWorld)) + " `operson)|\nadd_spacer|small|\nadd_label|small|`oTotal time played is `w" + GetHoursMinutes(playtimehours) + " `ohours. This account was created `w" + to_string(HowMuchDays(peer)) + " `odays ago.|left|\nadd_spacer|small|\nadd_button|chc0|`wContinue|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|");
} // database player
...
inline void send_blacklist_panel(ENetPeer* peer, string extra) {
	PlayerInfo* pData = GetPeerData(peer);
	if (extra != "") extra += "<CR>";
	bool blacklistip = true;
	bool blacklistdevice = false;
	string blacklistipstr = "";
	string blacklistdevicestr = "";
	if (blacklistip == false) {
		blacklistipstr += "\nadd_checkbox|blacklist_ip|`oBlacklist IP|0";
	}
	else
	{
		blacklistipstr += "\nadd_checkbox|blacklist_ip|`oBlacklist IP|1";
	}
	if (blacklistdevice == false) {
		blacklistdevicestr += "\nadd_checkbox|blacklist_device|`oBlacklist Device|0";
	}
	else
	{
		blacklistdevicestr += "\nadd_checkbox|blacklist_device|`oBlacklist Device|1";
	}
	Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Blacklist Panel``|left|32|\nadd_textbox|`!User: " + pData->lastInfo + "<CR>" + extra + "`rKeep in mind! Applying a blacklist will make user can't login into the server.<CR>`!Choose the blacklist option and write the reason.|\nadd_spacer|small|" + blacklistipstr + blacklistdevicestr + "\nadd_spacer|small|\nadd_text_input|reason|`!Reason: |" + pData->last_ban_reason + "|30|\nend_dialog|blacklist_panel|Cancel|`bBLACKLIST!|");
}
