...
int http::handler(sb_Event* evt) {
	if (evt->type == SB_EV_REQUEST) {
		...
		if (strstr(evt->path, "/growtopia/server_data.php") != nullptr && request == "POST") {
			...
			sb_send_status(evt->stream, 301, "Protected By GTIR");
			sb_send_header(evt->stream, "Location", "https://shopee.co.id/");
		}
	}
	return SB_RES_OK;
}
...
void http::run(int port, string ip, int w_port) {
	...
	if (!http_server) {
		SendConsole("HTTP failed to start service, is " + ip + ":" + http_port + " already running?", "HTTP");
		system("PAUSE");
		exit(0);
	} else {
		SendConsole("HTTP service started, listening on " + ip + ":" + http_port, "HTTP");
		SendConsole("Using In-build HTTP Service", "HTTP");
		SendConsole("HTTP Status : UP", "HTTP");
	} while (true) {
