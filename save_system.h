...
inline void save_manager() {
	while (true) {
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (worlds.at(i).name != "EXIT" && worlds.at(i).name != "error") {
					try {
						...
						for (int i = 0; i < square; i++) {
							...
							tile["text"] = info.items.at(i).text;
							...
							for (int Vip = 0; Vip < info.items.at(i).viplist.size(); Vip++) {
								tile["VipAcc"].push_back(info.items.at(i).viplist[Vip]);
							}
							tile["VipPublic"] = info.items.at(i).VipPublic;
							tile["DoorLocked"] = info.items.at(i).DoorLocked;
							tile["gtlab1"] = info.items.at(i).gateLabel1;
							tile["gtlab2"] = info.items.at(i).gateLabel2;
							tile["gtlab3"] = info.items.at(i).gateLabel3;
							tile["gtlab4"] = info.items.at(i).gateLabel4;
							tile["gtbt1"] = info.items.at(i).gateBt1;
							tile["gtbt2"] = info.items.at(i).gateBt2;
							tile["gtbt3"] = info.items.at(i).gateBt3;
							tile["gtbt4"] = info.items.at(i).gateBt4;
							tile["gtd1"] = info.items.at(i).destGate1;
							tile["gtd2"] = info.items.at(i).destGate2;
							tile["gtd3"] = info.items.at(i).destGate3;
							tile["gtd4"] = info.items.at(i).destGate4;
							tile["gtdid1"] = info.items.at(i).destGateId1;
							tile["gtdid2"] = info.items.at(i).destGateId2;
							tile["gtdid3"] = info.items.at(i).destGateId3;
							tile["gtdid4"] = info.items.at(i).destGateId4;
							tile["ds1"] = info.items.at(i).display1;
							tile["ds2"] = info.items.at(i).display2;
							tile["ds3"] = info.items.at(i).display3;
							tile["ds4"] = info.items.at(i).display4;
						...
						j["tempo"] = info.tempo;
						j["AllowNoClip"] = info.AllowNoClip;
						j["tiles"] = WorldTiles;
			...
			SendConsole("Worlds are now saved! server save data as requested by one player", "INFO");
		}
	}
}