...
struct PlayerInfo {
	...
	bool AdventureNotStarted = false;
	int billboardid = 0;
	int billboardtab = 0;
	...
	// BIRTH
	bool BirthCooldown = false;
	long long int BirthTime = 0;
	string newnames = "";
	//BIRTH END
...
public:
	...
	static void onMinGems(ENetPeer* peer, int gems);
	...
	try {
		...
		for (auto i = 0; i < static_cast<PlayerInfo*>(peer->data)->inventory.items.size(); i++) {
			if (static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemID == fItemid && static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemCount >= fQuantity) {
				iscontains = true;
				to_string(fQuantity) = static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemCount;
				break;
			}
		}
	}
...
inline void PlayerCharm(ENetPeer* peer, int colum1, int colum2, int colum3, int colum4, int colum5, int colum6, int berapapacket)
{
		...
		++currentPeer)
	{
		...
		if (isHere(peer, currentPeer))
		{
			if (berapapacket == 2)
			{
				GamePacket p3 = packetEnd(appendIntx(appendIntx(appendString(createPacket(), "OnDaylightDragon"), colum1), colum2));
			...
			{
				GamePacket p3 = packetEnd(appendIntx(appendIntx(appendIntx(appendIntx(appendIntx(appendIntx(appendString(createPacket(), "OnDaylightDragon"), colum1), colum2), colum3), colum4), colum5), colum6));
				memcpy(p3.data + 8, &(((PlayerInfo*)(peer->data))->netID), 4);
				ENetPacket* packet2 = enet_packet_create(p3.data, p3.len, ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(currentPeer, 0, packet2);
				delete p3.data;
...
inline void sendPuncheffectpeer(ENetPeer* peer, const int punch) {
	...
	data.punchY = -1;
	data.XSpeed = 300;
	if (static_cast<PlayerInfo*>(peer->data)->cloth_necklace == 10246)
	{
		if (((PlayerInfo*)(peer->data))->ef1 == true)
		{
			PlayerCharm(peer, 0, 0, 1, 10, 0, 1, 6);
		}
		if (((PlayerInfo*)(peer->data))->ef2 == true)
		{
			PlayerCharm(peer, 1, 0, 1, 10, 0, 1, 6);
		}
		if (((PlayerInfo*)(peer->data))->ef3 == true)
		{
...
inline void SendConsoleMsgAll(ENetPeer* peer, string message) {
...
inline bool NoLegendLevel(ENetPeer* peer) {
	if (static_cast<PlayerInfo*>(peer->data)->level >= 40) return true;
	else return false;
}
...
inline void save_playerinfo(PlayerInfo* pData) {
	...
	try {
		...
		j["firesOutput"] = pData->firesOutput;
		j["guild"] = pData->guild;
		j["joinguild"] = pData->joinguild;
		...
		j["birthcooldown"] = pData->BirthTime;
		...
		j["billboardid"] = pData->billboardid;
		j["billboardtab"] = pData->billboardtab;
		j["billboardprice"] = pData->billboardprice;
		j["billboardperwhat"] = pData->billboardperwhat;
...
inline int PlayerDB::playerRegister(ENetPeer* peer, string username, string password, string email) {
	...
	if (existsRegisteringIP) {
		...
		getIp.close();
		if (registeringIpCount > 5) return -6;
	...
	else
	{
		...
		j["verified"] = true;
		j["ipblacklisted"] = false;
		j["deviceblacklisted"] = false;
		j["gems"] = 0;
		j["lastworld"] = "";
		j["timecursed"] = 0;
		j["timemuted"] = 0;
...
void ThreadPlaytime(ENetPeer* peer) {
	while (1) {
		...
		ofstream savebantime2("save/playtime/_" + ((PlayerInfo*)(peer->data))->rawName + ".txt");
		savebantime2 << playtime1 + 1 << endl;
		savebantime2.close();
	}
}

int countplaytime(int y, int m, int d) {
	if (m < 3) y--, m += 12;
	return 365 * y + y / 4 - y / 100 + y / 400 + (153 * m - 457) / 5 + d - 306;
}

int HowMuchDays(ENetPeer* peer) {
	int playtimeyears = 0;
	int playtimemonths = 0;
	int playtimedays = 0;
	ifstream fd("save/registered/_" + ((PlayerInfo*)(peer->data))->rawName + ".txt");
	fd >> playtimeyears;
	fd >> playtimemonths;
	fd >> playtimedays;
	fd.close();
	...
	const auto Day = localTime->tm_mday;
	const auto Month = localTime->tm_mon + 1;
	int days = countplaytime(Year, Month, Day) - countplaytime(playtimeyears, playtimemonths, playtimedays);
	return days;
}

string GetHoursMinutes(int n) {
	int playtimehours = n / 3600;
	n %= 3600;
	int playtimeminutes = n / 60;
	return to_string(playtimehours) + "." + to_string(playtimeminutes);
}
...
#pragma warning(disable: 4996)
	...
	string Sic = "";
	if (Hour < 12 && Min == 00) Sic = "AM";
	else Sic = "PM";
	string checkmin = to_string(Min);
	if (checkmin.length() == 1) Min = 0 + localTime->tm_min;
	string monthname = "January";
	string ordinalnumber = "st";
	if (Mon == 1) monthname = "January";
	else if (Mon == 2) monthname = "February";
	else if (Mon == 3) monthname = "March";
	else if (Mon == 4) monthname = "April";
	else if (Mon == 5) monthname = "May";
	else if (Mon == 6) monthname = "June";
	else if (Mon == 7) monthname = "July";
	else if (Mon == 8) monthname = "August";
	else if (Mon == 9) monthname = "September";
	else if (Mon == 10) monthname = "October";
	else if (Mon == 11) monthname = "November";
	else if (Mon == 12) monthname = "December";
	string ordinalnumber2 = to_string(Day);
	if (ordinalnumber2 == "1") ordinalnumber = "st";
	else if (ordinalnumber2 == "2") ordinalnumber = "nd";
	else if (ordinalnumber2 == "3") ordinalnumber = "rd";
	else if (ordinalnumber2 == "21") ordinalnumber = "st";
	else if (ordinalnumber2 == "22") ordinalnumber = "nd";
	else if (ordinalnumber2 == "23") ordinalnumber = "rd";
	else if (ordinalnumber2 == "31") ordinalnumber = "st";
	else ordinalnumber = "th";
	return monthname + " " + to_string(Day) + ordinalnumber + "," + to_string(Hour) + ":" + to_string(Min);
}
...
inline void showModLogs(string textInfo) {
	...
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		if (((PlayerInfo*)(currentPeer->data))->currentWorld == "EXIT") continue;
		if (((PlayerInfo*)(currentPeer->data))->isIn == false) continue;
		int adminLevel = static_cast<PlayerInfo*>(currentPeer->data)->adminLevel;
		string plrRole = role_nameko.at(adminLevel);
		if (plrRole == "owner" || plrRole == "owner") {
...
inline void Player::onMinGems(ENetPeer* peer, const int gems)
{
	int currentgems = 0;
	ifstream fs("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
	fs >> currentgems;
...
inline void FailLogin(ENetPeer* peer, string text, bool withurl) {
	if (withurl) {
		const string dc = server_discord;
		const auto url = "action|set_url\nurl|" + dc + "\nlabel|Join " + server_name + " Discord Server\n";
		const auto data = new BYTE[5 + text.length()];
		const auto dataurl = new BYTE[5 + url.length()];
		BYTE zero = 0;
		auto type = 3;
