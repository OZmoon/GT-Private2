...
inline void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer, WorldInfo* world) {
	...
	bool isTree = false, isLock = false, isSmallLock = false, isHeartMonitor = false, isMannequin = false, isScience = false, isMagplant = false, isgateway = false, iscontains = false, VendUpdate = false, DigiVendUpdate = false, isDoor = false;
	...
	try {
		...
		case 18: /*punch*/
			...
			// GEIGER CHARGER
			if (world->items.at(x + (y * world->width)).foreground == 4654) {
				if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "") {
					if (tile == 2204) {
						Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "`wYour Geiger Counter doesn't need charging!", 0, false);
						return;
					}
					else if (tile == 2286) {
						RemoveInventoryItem(2286, 1, peer, true);
						for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
							if (isHere(peer, currentPeer)) {
								SendThrowEffect(currentPeer, 2286, ((PlayerInfo*)(peer->data))->netID, -1, 150, 0, x * 32 + 16, y * 32 + 16);
								SendGeigerCharger(currentPeer, 4654, 0, x, y, 1);
							}
						}
					}
					else {
				...
				if (pData->cloth_hand == 3066) {
					if (world->items.at(x + (y * world->width)).fire) {
						...
						static_cast<PlayerInfo*>(peer->data)->firesOutput += 1;
				...
				if (getItemDef(world->items.at(x + (y * world->width)).foreground).blockType == BlockTypes::PROVIDER) {
					if (world->owner == "" || pData->rawName == PlayerDB::getProperName(world->owner) || isDev(peer)) {
						if (calcBanDuration(world->items.at(x + (y * world->width)).growtime) == 0) {
							...
							if (world->items.at(x + (y * world->width)).foreground == 8522) {
								vector<int> list{ 0, 2734, 0, 2732, 0, 2746, 0, 2750, 0, 4752, 0, 4770, 0, 4772, 0, 8522, 8522, 0 };
				...
				string gay = world->items.at(x + (y * world->width)).text;
				int gay2 = world->items.at(x + (y * world->width)).foreground;
				if (world->items.at(x + (y * world->width)).foreground == 4722)
				{
					if (world->owner != "") {
		...
		case 32: /*wrench*/
		{
			...
			if (world->items.at(x + (y * world->width)).foreground == 5958) {
				if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y)) {
					...
					Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wEpoch Machine|left|5958|\nadd_spacer|small|\nadd_textbox|`oSelect your doom:|left|\nadd_checkbox|iceage|`oIce Age|\nadd_checkbox|volcano|`oVolcono|\nadd_checkbox|sky|`oFloating island|\nadd_text_input|timeepoch|`oClye time (minutes):|10|5|\nend_dialog|epoch_weather|Cancel|Okay|");
				}
				else if (world->owner != "") {
					Player::OnTalkBubble(peer, pData->netID, "That area is owned by " + world->owner + "", 0, true);
				}
			...
			// XENONITE CRYSTAL
			if (world->items.at(x + (y * world->width)).foreground == 2072)
			{
				if (isWorldOwner(peer, world) || world->owner == "") {
					//double jump
					string block_jump = "0";
					//if (world->blockedjump) block_jump = "1";
					string allow_jump = "0";
				//	if (world->allowedjump) allow_jump = "1";
					//high jump
					string block_highjump = "0";
				//	if (world->blockedhighjump) block_highjump = "1";
					string allow_highjump = "0";
				//	if (world->allowedhighjump) allow_highjump = "1";
					//strong punch
					string block_strongpunch = "0";
				//	if (world->blockedstrongpunch) block_strongpunch = "1";
					string allow_strongpunch = "0";
				//  if (world->allowedstrongpunch) allow_strongpunch = "1";
					//speed
					string block_speed = "0";
				//	if (world->blockedspeed) block_speed = "1";
					string allow_speed = "0";
				//	if (world->allowedspeed) allow_speed = "1";
					Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_label_with_icon|big|`wXenonite Crystal`|left|2072|\n\nadd_spacer|small|\nadd_textbox|`oThis crystal can either grant or block super powers for all players in your world! Any power that's unchecked will work as normal - people will have the power if they equip an item with it.|\nadd_spacer|small|\nadd_checkbox|checkbox_allowjump|`oForce Double Jump|" + allow_jump + "|\nadd_spacer|small|\nend_dialog|xenonite_update|`oCancel|`wUpdate|\n");
				}
				return;
			}			// OVEN
			if (world->items.at(x + (y * world->width)).foreground == 952) {
				if (isWorldOwner(peer, world) || world->owner == "" || isWorldAdmin(peer, world) || world->isPublic) {
					if (tile == 18) {
					if (world->items.at(x + (y * world->width)).activated) {
						SendCombiners(peer, x, y, 1096, 1098, 1828, 4, 4, 6, 1056, true); /*Songpyeon*/
						SendCombiners(peer, x, y, 1056, 1096, 1828, 15, 30, 15, 1804, true); /*Zeus Lightning Bolt*/
						SendCombiners(peer, x, y, 611, 874, 0, 100, 15, 0, 1552, false); /*Pet Bunny Recipe*/
						SendCombiners(peer, x, y, 1056, 1804, 3870, 5, 10, 15, 1824, true); /*AutumnWings*/
						SendCombiners(peer, x, y, 1056, 7058, 10030, 3, 10, 1, 1826, true); /*AutumnBomb*/
						SendCombiners(peer, x, y, 1056, 1804, 10030, 1, 5, 1, 3074, true); /*FloatingLeaf*/
						SendCombiners(peer, x, y, 10322, 10324, 10326, 1, 1, 1, 10328, false); /*Dark Kings Offering*/
						SendCombiners(peer, x, y, 898, 4538, 5018, 2, 1, 1, 5022, false); /*Chicken Plow*/
						SendCombiners(peer, x, y, 2274, 5026, 5028, 1, 5, 20, 5060, false); /*Verdant Gate*/
						SendCombiners(peer, x, y, 2274, 5028, 5024, 1, 5, 20, 5054, false); /*Inferno Gate*/
						SendCombiners(peer, x, y, 2274, 5024, 5030, 1, 5, 20, 5058, false); /*Typhoon Gate*/
						SendCombiners(peer, x, y, 2274, 5030, 5026, 1, 5, 20, 5056, false); /*Tidal Gate*/
						SendCombiners(peer, x, y, 2492, 5028, 5026, 1, 20, 5, 5050, false); /*Earth Asteroid*/
						SendCombiners(peer, x, y, 2492, 5024, 5028, 1, 20, 5, 5046, false); /*Fire Asteroid*/
						SendCombiners(peer, x, y, 2492, 5030, 5024, 1, 20, 5, 5052, false); /*Wind Asteroid*/
						SendCombiners(peer, x, y, 2492, 5026, 5030, 1, 20, 5, 5048, false); /*Water Asteroid*/
						SendCombiners(peer, x, y, 3946, 5060, 5028, 100, 4, 5, 5070, false); /*Crystallized Reality*/
						SendCombiners(peer, x, y, 3838, 5054, 5024, 200, 4, 5, 5072, false); /*Crystallized Wealth*/
						SendCombiners(peer, x, y, 340, 5058, 5030, 200, 4, 5, 5074, false); /*Crystallized Brilliance*/
						SendCombiners(peer, x, y, 2272, 5056, 5026, 30, 4, 5, 5076, false); /*Crystallized Nature*/
						SendCombiners(peer, x, y, 5050, 5048, 5028, 2, 2, 5, 5066, false); /*Ancient Bracket*/
						SendCombiners(peer, x, y, 5046, 5052, 5024, 2, 2, 5, 5062, false); /*Ancient Hinge*/
						SendCombiners(peer, x, y, 5052, 5050, 5030, 2, 2, 5, 5068, false); /*Ancient Lens*/
						SendCombiners(peer, x, y, 5048, 5046, 5026, 2, 2, 5, 5064, false); /*Ancient Pin*/
						SendCombiners(peer, x, y, 362, 386, 1458, 1, 200, 1, 1460, false); /*Golden Angel Wings*/
						SendCombiners(peer, x, y, 364, 386, 1458, 1, 200, 1, 1462, false); /*Golden Diaper*/
						SendCombiners(peer, x, y, 1470, 386, 1458, 1, 200, 1, 1466, false); /*Golden Diamond Necklace*/
						SendCombiners(peer, x, y, 366, 386, 1458, 1, 200, 1, 1464, false); /*Golden Heartbow*/
						SendCombiners(peer, x, y, 2388, 386, 1458, 1, 20, 1, 2386, false); /*Heavenly Scythe*/
						SendCombiners(peer, x, y, 2390, 386, 1458, 1, 20, 1, 2392, false); /*Teeny Golden Wings*/
						SendCombiners(peer, x, y, 2384, 386, 1458, 1, 20, 1, 2394, false); /*Golden Love Bug*/
						SendCombiners(peer, x, y, 4412, 386, 1458, 1, 20, 1, 4414, false); /*Golden Pegasus*/
						SendCombiners(peer, x, y, 4416, 386, 1458, 1, 20, 1, 4420, false); /*Golden Heart Glasses*/
						SendCombiners(peer, x, y, 5644, 386, 1458, 1, 20, 1, 5646, false); /*Golden Heart Shirt*/
						SendCombiners(peer, x, y, 5648, 386, 1458, 1, 20, 1, 5650, false); /*Golden Talaria*/
						SendCombiners(peer, x, y, 7826, 386, 1458, 1, 200, 1, 7828, false); /*Golden Heartstaff*/
						SendCombiners(peer, x, y, 7830, 386, 1458, 1, 200, 1, 7832, false); /*Golden Heartsword*/
						SendCombiners(peer, x, y, 9320, 386, 1458, 1, 200, 1, 9322, false); /*Golden Sunset Cape*/
						SendCombiners(peer, x, y, 5070, 5066, 5064, 1, 2, 5, 5080, false); /*Ancestral Tesseract of Dimensions*/
						SendCombiners(peer, x, y, 5076, 5068, 5066, 1, 2, 5, 5082, false); /*Ancestral Seed of Life*/
						SendCombiners(peer, x, y, 5072, 5064, 5062, 1, 2, 5, 5084, false); /*Ancestral Lens of Riches*/
						SendCombiners(peer, x, y, 5074, 5062, 5068, 1, 2, 5, 5078, false); /*Ancestral Totem of Wisdom*/
						SendCombiners(peer, x, y, 7826, 7830, 1458, 1, 1, 1, 7834, false); /*Stained Glass Heartwings*/
						SendCombiners(peer, x, y, 9342, 9340, 1458, 1, 1, 1, 9344, false); /*Datemaster's Heart Locket*/
						SendCombiners(peer, x, y, 9324, 386, 1458, 1, 200, 1, 9326, false); /*Golden Heartthrob Helm*/
						SendCombiners(peer, x, y, 9314, 1800, 186, 1, 1, 200, 9316, false); /*Beefy Biceps*/
						SendCombiners(peer, x, y, 1506, 8794, 4406, 1, 1, 1, 9318, false); /*Snuggly Kitty Scarf*/
						SendCombiners(peer, x, y, 914, 916, 924, 5, 5, 10, 770, false); /*Combover*/
						SendCombiners(peer, x, y, 914, 916, 920, 30, 4, 1, 906, false); /*Shocking Hair*/
						SendCombiners(peer, x, y, 914, 916, 918, 10, 5, 1, 902, false, 3); /*Shockinator*/
						SendCombiners(peer, x, y, 914, 924, 920, 5, 4, 2, 926, false, 5); /*Steel Table*/
						SendCombiners(peer, x, y, 914, 924, 918, 6, 3, 4, 1114, false); /*Dragon Tail*/
						SendCombiners(peer, x, y, 914, 920, 918, 15, 4, 2, 978, false, 4); /*Slime*/
						SendCombiners(peer, x, y, 916, 924, 918, 10, 3, 3, 772, false); /*Lab Coat*/
						SendCombiners(peer, x, y, 916, 920, 918, 2, 4, 3, 904, false); /*Ze Goggles*/
						SendCombiners(peer, x, y, 924, 920, 918, 20, 10, 5, 922, false); /*Mysterious Chemical*/
						SendCombiners(peer, x, y, 914, 916, 922, 20, 10, 1, 1162, false); /*Forcefield*/
						SendCombiners(peer, x, y, 914, 920, 922, 10, 2, 1, 1746, false, 5); /*Fuel Pack*/
						SendCombiners(peer, x, y, 914, 918, 922, 20, 15, 10, 1542, false); /*Battle Trout*/
						SendCombiners(peer, x, y, 916, 924, 922, 8, 4, 1, 2588, false); /*Sassy Boots*/
						SendCombiners(peer, x, y, 916, 918, 922, 1, 1, 1, 1866, false); /*Block Glue*/
						SendCombiners(peer, x, y, 924, 920, 922, 3, 3, 3, 908, false); /*Enchanced Intellect*/
						SendCombiners(peer, x, y, 924, 918, 922, 4, 3, 1, 1310, false); /*Thingamajig*/
						SendCombiners(peer, x, y, 920, 918, 922, 200, 200, 200, 910, false); /*Reanimator Remote*/
						SendCombiners(peer, x, y, 914, 922, 1962, 10, 10, 10, 4582, false); /*Food Grinder*/
						SendCombiners(peer, x, y, 916, 924, 1962, 9, 5, 1, 4552, false); /*Synthetic Oculum*/
						UpdateBlockState(peer, x, y, true, world);
						world->items.at(x + (y * world->width)).activated = false;
					}
					else {
			...
			// FISH TANK PORT
			if (world->items.at(x + (y * world->width)).foreground == 3002) {
				if (isWorldOwner(peer, world)) {
					string lol = world->items[x + (y * world->width)].text, dialog = "";
					string cbox = "";
					...
					if (lol != "") {
						...
						if (gg.size() != 0) {
							for (auto& z : gg) {
								...
								cbox += "\nadd_checkbox|remfish_" + xd[0] + "_" + xd[1] + "_" + xd[2] + "|`o" + xd[1] + "lb. " + itemDefs[stoi(xd[0])].name + "|0";
							}
						}
			...
			if (world->items.at(x + (y * world->width)).foreground == 1790) { //mystic wizard
				if (!NoLegendLevel(peer)) {
					pData->lastPunchX = x;
					pData->lastPunchY = y;
					pData->choosing_quest = "";
					if (pData->lastquest == "" || !pData->quest_active) {
						Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`9The Legendary Wizard``|left|1790|\nadd_smalltext|`oGreetings, traveker! I am the Legendary Wizard. You should wish to embark on a Legendary Quest, simply choose one bellow.|\nadd_spacer|small|\nadd_smalltext|`oOh wait, sorry... looks like you aren't yet legendary enough for Legendary Quests. You must be at least Level 40 to begin your journey!|\nadd_smalltext|`oBut hey, since you can't do a quest, maybe there's something else you can do. Step a little closer...|\nadd_button|lwizlvl|`9Yes?|\nend_dialog|legendary_wizard_no_level|`oDarn it||\n");
					}
				}
				else if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y) && pData->level >= 39) { //wizard
					pData->lastPunchX = x;
					pData->lastPunchY = y;
					pData->choosing_quest = "";
					if (pData->lastquest == "" || !pData->quest_active) { //legendary wizard
						Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`9The Legendary Wizard``|left|1790|\nadd_textbox|`oGreetings, traveler! I am the Legendary Wizard. Should you wich to embark on a Legendary Quest, simply choose one below.|\nadd_spacer|small|\nadd_button|honor|`9Quest For Honor|noflags|0|0|\nadd_button|steel|`9Quest For Steel|noflags|0|0|\nadd_button|fire|`9Quest For Fire|noflags|0|0|\nadd_button|heavens|`9Quest For Heavens|noflags|0|0|\nadd_button|blade|`9Quest For Blade|noflags|0|0|\nadd_button|candour|`9Quest For Candour|noflags|0|0|\nadd_button|thesky|`9Quest For The Sky|noflags|0|0|\nend_dialog|legendary_wizard|No Thanks||\n");
					}
					else if (pData->lastquest == "honor") {
					...
					if (isWorldOwner(peer, world) || isDev(peer) || pData->rawName == world->items.at(x + (y * world->width)).monitorname) {
						...
						if (world->category == "Guild") {
							...
							try {
								...
								read_guild >> js;
								read_guild.close();
								int guild_fg = js["foregroundflag"];
								int guild_bg = js["backgroundflag"];
								...
								abandon_guild = "\nadd_button|confirmguildleaderleave|`4Abandon " + guild_name + " guild``|noflags|0|0|";
				...
				if (world->items.at(x + (y * world->width)).foreground == 3818) {
					if (isWorldOwner(peer, world) || isWorldAdmin(peer, world)) {
						string dialog = "", ok = world->items.at(x + (y * world->width)).text;
						if (world->items.at(x + (y * world->width)).text == "") {
				...
				if (world->items.at(x + (y * world->width)).foreground == 3794 || world->items.at(x + (y * world->width)).foreground == 10190 || world->items.at(x + (y * world->width)).foreground == 10552) {
					if (world->owner == "" || isWorldOwner(peer, world) || isWorldAdmin(peer, world) || getPlyersWorld(peer)->isPublic) {
						...
						if (world->items.at(x + (y * world->width)).display1 != 0) {
							d1 = "\nadd_item_picker|ds1|`oItem 1: `2" + getItemDef(world->items.at(x + (y * world->width)).display1).name + "|Select any item to use|";
						}
						else {
							d1 = "\nadd_item_picker|ds1|`4Display an item|Select any item to use|";
						}
						if (world->items.at(x + (y * world->width)).display2 != 0) {
							d2 = "\nadd_item_picker|ds2|`oItem 2: `2" + getItemDef(world->items.at(x + (y * world->width)).display2).name + "|Select any item to use|";
						}
						else {
							d2 = "\nadd_item_picker|ds2|`4Display an item|Select any item to use|";
						}
						if (world->items.at(x + (y * world->width)).display3 != 0) {
							d3 = "\nadd_item_picker|ds3|`oItem 3: `2" + getItemDef(world->items.at(x + (y * world->width)).display3).name + "|Select any item to use|";
						}
						else {
							d3 = "\nadd_item_picker|ds3|`4Display an item|Select any item to use|";
						}
						if (world->items.at(x + (y * world->width)).display4 != 0) {
							d4 = "\nadd_item_picker|ds4|`oItem 4: `2" + getItemDef(world->items.at(x + (y * world->width)).display4).name + "|Select any item to use|";
						}
						else {
							d4 = "\nadd_item_picker|ds4|`4Display an item|Select any item to use|";
						}
						Player::OnDialogRequest(peer, "add_label_with_icon|big|`w" + getItemDef(world->items.at(x + (y * world->width)).foreground).name + "|left|" + std::to_string(world->items.at(x + (y * world->width)).foreground) + "|\nadd_spacer|small|" + d1 + "|" + d2 + "|" + d3 + "|" + d4 + "|\nend_dialog|displayself|Okay||");
						static_cast<PlayerInfo*>(peer->data)->wrenchedBlockLocation = x + (y * world->width);
					}
					else {
						string d1 = ""; string d2 = ""; string d3 = ""; string d4 = "";
						if (world->items.at(x + (y * world->width)).display1 != 0) {
							d1 = "\nadd_smalltext|`2" + getItemDef(world->items.at(x + (y * world->width)).display1).name + "|";
						}
						if (world->items.at(x + (y * world->width)).display2 != 0) {
							d2 = "\nadd_smalltext|`2" + getItemDef(world->items.at(x + (y * world->width)).display2).name + "|";
						}
						if (world->items.at(x + (y * world->width)).display3 != 0) {
							d3 = "\nadd_smalltext|`2" + getItemDef(world->items.at(x + (y * world->width)).display3).name + "|";
						}
						if (world->items.at(x + (y * world->width)).display4 != 0) {
							d4 = "\nadd_smalltext|`2" + getItemDef(world->items.at(x + (y * world->width)).display4).name + "|";
						}
						if (world->items.at(x + (y * world->width)).display1 == 0 && world->items.at(x + (y * world->width)).display2 == 0 && world->items.at(x + (y * world->width)).display3 == 0 && world->items.at(x + (y * world->width)).display4 == 0) {
							Player::OnDialogRequest(peer, "add_label_with_icon|big|`w" + getItemDef(world->items.at(x + (y * world->width)).foreground).name + "|left|" + std::to_string(world->items.at(x + (y * world->width)).foreground) + "\nadd_spacer|small|\nadd_textbox|`oItems on display here:|left|\nadd_smalltext|`oNone!|left||\nend_dialog|displayself|Okay||");
							static_cast<PlayerInfo*>(peer->data)->wrenchedBlockLocation = x + (y * world->width);
						}
						else {
							Player::OnDialogRequest(peer, "add_label_with_icon|big|`w" + getItemDef(world->items.at(x + (y * world->width)).foreground).name + "|left|" + std::to_string(world->items.at(x + (y * world->width)).foreground) + "\nadd_spacer|small|\nadd_textbox|`oItems on display here:|left|" + d1 + "|" + d2 + "|" + d3 + "|" + d4 + "|\nend_dialog|displayself|Okay||");
							static_cast<PlayerInfo*>(peer->data)->wrenchedBlockLocation = x + (y * world->width);
						}
					}
				}
				if (world->items.at(x + (y * world->width)).foreground == 1436) {
					if (world->owner == "" || isWorldOwner(peer, world) || isWorldAdmin(peer, world)) {
						...
						if (!cctv) {
							ofstream of("save/cctv/_" + ((PlayerInfo*)(peer->data))->currentWorld + ".json");
							vector<string> logs;
							json j;
							j["logs"] = logs;
							...
							if (log == "") cctv.addTextBox("`oThe camera log is empty.");
							else cctv.addSpacer(SPACER_SMALL);
							if (j["ShowItemTake"] == true) cctv.addCheckbox("enableitemtake", "`oShow Item Taking", CHECKBOX_SELECTED);
							else cctv.addCheckbox("enableitemtake", "`oShow Item Taking", CHECKBOX_NOT_SELECTED);
							if (j["ShowItemDrop"] == true) cctv.addCheckbox("enableitemdrop", "`oShow Item Dropping", CHECKBOX_SELECTED);
							else cctv.addCheckbox("enableitemdrop", "`oShow Item Dropping", CHECKBOX_NOT_SELECTED);
							if (j["ShowPoepleEnter"] == true) cctv.addCheckbox("enablepenter", "`oShow Poeple Entering", CHECKBOX_SELECTED);
							else cctv.addCheckbox("enablepenter", "`oShow People Entering", CHECKBOX_NOT_SELECTED);
							if (j["ShowPoepleExit"] == true) cctv.addCheckbox("enablepleave", "`oShow Poeple Exiting", CHECKBOX_SELECTED);
							else cctv.addCheckbox("enablepleave", "`oShow People Exiting", CHECKBOX_NOT_SELECTED);
							if (j["DontShowOwner"] == true) cctv.addCheckbox("enableDontShowOwner", "`oDon't Show Owner", CHECKBOX_SELECTED);
							else cctv.addCheckbox("enableDontShowOwner", "`oDon't Show Owner", CHECKBOX_NOT_SELECTED);
							if (log != "") cctv.addButton("clearcctvlog", "`4Clear Logs");
							else cctv.addSpacer(SPACER_SMALL);
							cctv.addQuickExit();
						...
						Player::OnDialogRequest(peer, "set_default_color|\nadd_label_with_icon|big|`wSecurity Camera|left|1436|\nadd_spacer|small|\nadd_textbox|`oCamera logs can only be read by their owner!|left|\nadd_button|cctvupdate|`wOk|");
						return;
					}
				}
				if (world->items.at(x + (y * world->width)).foreground == 2646) {
					((PlayerInfo*)(peer->data))->lastPunchX = x;
					((PlayerInfo*)(peer->data))->lastPunchY = y;
					string spotedTo = world->items.at(x + (y * world->width)).spotTo;
					
					if (((PlayerInfo*)(peer->data))->rawName == world->owner) {
						if (spotedTo == "") {
							Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wShine the Spotlight!|left|2646|\nadd_spacer|small|\nadd_textbox|`oThe light is currently off.|\nadd_spacer|small|\nadd_player_picker|shinespot|`wChoose a superstar``|\nend_dialog|spotlight|Nevermind|");
						}
						else
						{
							Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wShine the Spotlight!|left|2646|\nadd_spacer|small|\nadd_textbox|`oThe light is shining on " + spotedTo + ".|\nadd_player_picker|shinespot|`wChoose a new star``|\nadd_button|turnspotoff|Turn spotlight off|\nend_dialog|spotlight|Nevermind|");
						}
					}
					else
					{
						if (spotedTo == ((PlayerInfo*)(peer->data))->rawName) {
							Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wShine the Spotlight!|left|2646|\nadd_spacer|small|\nadd_textbox|`oThe light is shining on you!|\nadd_spacer|small|\nend_dialog|spotlight|Nevermind|");
						}
					}
				}
				if (world->items.at(x + (y * world->width)).foreground == 3798) {
					((PlayerInfo*)(peer->data))->lastPunchX = x;
					((PlayerInfo*)(peer->data))->lastPunchY = y;
					if ((find(world->items.at(x + (y * world->width)).viplist.begin(), world->items.at(x + (y * world->width)).viplist.end(), ((PlayerInfo*)(peer->data))->rawName) != world->items.at(x + (y * world->width)).viplist.end())) {
						Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_label_with_icon|big|`wEdit " + getItemDef(world->items.at(x + (y * world->width)).foreground).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).foreground) + "|\nadd_textbox|`oYou're on the VIP list!|left|\nadd_spacer|small|\nend_dialog|RemoveMyVipAccess|Cancel|`wRemove my Access|");
						return;
					}
					if (((PlayerInfo*)(peer->data))->rawName == world->owner) {
						...
						if (world->items.at(x + (y * world->width)).VipPublic == true) ischecked = 1;
						else ischecked = 0;
						string VipPublic = to_string(ischecked);
						if (world->items.at(x + (y * world->width)).viplist.size() == 0) {
							((PlayerInfo*)(peer->data))->lastPunchX = x;
							((PlayerInfo*)(peer->data))->lastPunchY = y;
						...
						if (world->items.at(x + (y * world->width)).VipPublic == true) {
				...
				if (world->items.at(x + (y * world->width)).foreground == 5086) {
					//Upgrade Ancestral Coded by - GTIR//
					if (isWorldOwner(peer, world) || isWorldAdmin(peer, world) || isDev(peer) || world->owner == "" || !restricted_area(peer, world, x, y)) {
						bool checkForPlr = std::experimental::filesystem::exists("save/ances/currentquest/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
						if (checkForPlr == false) {
							ofstream createTxtFile("save/ances/currentquest/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							createTxtFile.close();
							ofstream createTxtFile2("save/ances/currentqueststep/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							createTxtFile2.close();
							ofstream fd("save/ances/currentqueststep/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							fd << 1;
							fd.close();
							Player::OnDialogRequest(peer, "add_label_with_icon|big|`9Ancient Goddess``|left|5086|\nadd_spacer|small|\nadd_textbox|You can upgrade your ancestral items here!|\nadd_spacer|small|\nadd_button|upwis|Upgrade Ancestral Totem of Wisdom|\nadd_button|upric|Upgrade Ancestral Lens of Riches|\nadd_button|uplif|Upgrade Ancestral Seed of Life|\nadd_button|uptim|Upgrade Ancestral Orb of Time|\nadd_button|updim|Upgrade Ancestral Tesseract of Dimensions|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
						}
						else {
							string plrQuest = "";
							int plrStep = 0;
							ifstream checkPlrQuest("save/ances/currentquest/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							checkPlrQuest >> plrQuest;
							checkPlrQuest.close();
							ifstream checkPlrStep("save/ances/currentqueststep/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							checkPlrStep >> plrStep;
							checkPlrStep.close();
							if (plrQuest == "wisdom") {
								if (plrStep == 1) {
									...
									int plrLevel = static_cast<PlayerInfo*>(peer->data)->level;
									if (plrLevel >= 15) {
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oI want you to reach level 15 in this wonderful GrowPS Server!|\nadd_smalltext|`oYour current level : " + to_string(plrLevel) + "|left|\nadd_spacer|small|\nadd_textbox|`oSeems like you've got enough level..|\nadd_button|claimwis2|Upgrade!|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}
									else if (plrLevel < 15)
									{
										int levelShort = 15 - plrLevel;
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oI want you to reach level 15 in this wonderful GrowPS Server!|\nadd_smalltext|`oYour current level : " + to_string(plrLevel) + "|left|\nadd_spacer|small|\nadd_textbox|`oIt seems like you've worked hard, but not enough...|\nadd_smalltext|You are " + to_string(levelShort) + " Level left to reach Level 15..|left|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}
								}
								else if (plrStep == 4) {
									...
									SearchInventoryItem(peer, id, 1, iscontain);
									if (iscontain) {
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oI want you to show me your Angel of Mercy's Wings.|\nadd_spacer|small|\nadd_label_with_icon|small|`0Angel Of Mercy's Wings|left|8552|\nadd_spacer|small|\nadd_textbox|`oI've seen the Angel of Mercy's Wings in your backpack..|\nadd_smalltext|`oYou have " + to_string(quantityinBP) + " Angel of Mercy's Wings in your backpack.|left|\nadd_spacer|small|\nadd_button|claimwis3|Upgrade!|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}
									else
									{
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oI want you to show me your Angel of Mercy's Wings.|\nadd_spacer|small|\nadd_label_with_icon|small|`0Angel Of Mercy's Wings|left|8552|\nadd_spacer|small|\nadd_textbox|`oI didn't find any Angel of Mercy's Wings in your backpack.. go find one.|\nadd_smalltext|`oYou have " + to_string(quantityinBP) + " Angel of Mercy's Wings in your backpack.|left|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}
								}
								if (plrStep == 5) {
									auto quantityinBP = 0;
									...
									SearchInventoryItem(peer, id, quantity, iscontain1);
									if (iscontain1 && plrGems >= quantity2) {
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oWelcome to the Ancient Goddess child|\nadd_smalltext|`oI want you to deliver me:|left|\nadd_label_with_icon|small|`2" + to_string(quantity) + " " + getItemDef(id).name + "|" + "left" + "|" + to_string(id) + "|\nadd_smalltext|and|left|\nadd_label_with_icon|small|`2 " + to_string(quantity2) + " " + getItemDef(id2).name + "|" + "left" + "|" + to_string(id2) + "|\nadd_spacer|small|\nadd_smalltext|`6(You have " + to_string(quantityinBP) + " " + getItemDef(id).name + " and " + to_string(plrGems) + " " + getItemDef(id2).name + "``)|left|\nadd_textbox|`oLooks like you got them all in your backpack, child come give me those items and gems!|\nadd_spacer|small|\nadd_button|claimwis4|Upgrade!|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}

									if (!iscontain1 && plrGems < quantity2) {
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oWelcome to the Ancient Goddess child|\nadd_smalltext|`oI want you to deliver me:|left|\nadd_label_with_icon|small|`2" + to_string(quantity) + " " + getItemDef(id).name + "|" + "left" + "|" + to_string(id) + "|\nadd_smalltext|and|left|\nadd_label_with_icon|small|`2" + to_string(quantity2) + " " + getItemDef(id2).name + "|" + "left" + "|" + to_string(id2) + "|\nadd_spacer|small|\nadd_smalltext|`6(You have " + to_string(quantityinBP) + " " + getItemDef(id).name + " and " + to_string(plrGems) + " " + getItemDef(id2).name + "``)|left|\nadd_textbox|`oLooks like you don't have the requirements!, Come back when you get 'em.|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}

									if (quantityinBP < quantity || plrGems < quantity2) {
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Totem of Wisdom|left|5078|\nadd_spacer|small|\nadd_textbox|`oWelcome to the Ancient Goddess child|\nadd_smalltext|`oI want you to deliver me:|left|\nadd_label_with_icon|small|`2" + to_string(quantity) + " " + getItemDef(id).name + "|" + "left" + "|" + to_string(id) + "|\nadd_smalltext|and|left|\nadd_label_with_icon|small|`2" + to_string(quantity2) + " " + getItemDef(id2).name + "|" + "left" + "|" + to_string(id2) + "|\nadd_spacer|small|\nadd_smalltext|`6(You have " + to_string(quantityinBP) + " " + getItemDef(id).name + " and " + to_string(plrGems) + " " + getItemDef(id2).name + "``)|left|\nadd_textbox|`oLooks like you don't have the requirements!, Come back when you get 'em.|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}
								}
							}
							else if (plrQuest == "riches") {
								if (plrStep == 1) {
									...
									else
									{
										...
										{
											Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Lens of Riches|left|5084|\nadd_spacer|small|\nadd_textbox|`oYou don't seem to have Ancestral Lens of Riches, come back here if you got one child.|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
											ofstream fd("save/ances/currentquest/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
											fd << "";
											fd.close();
										}
									...
									int plrLevel = static_cast<PlayerInfo*>(peer->data)->level;
									if (plrLevel >= 15) {
										Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Lens of Riches|left|5084|\nadd_spacer|small|\nadd_textbox|`oI want you to reach level 15 in this wonderful " + server_name + " Server!|\nadd_smalltext|`oYour current level : " + to_string(plrLevel) + "|left|\nadd_spacer|small|\nadd_textbox|`oSeems like you've got enough level..|\nadd_button|claimric2|Upgrade!|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
									}
									else if (plrLevel < 15)
									{
										int levelShort = 15 - plrLevel;
								...
								if (plrStep == 1) {
									...
									else
									{
										...
										{
											Player::OnDialogRequest(peer, "add_label_with_icon|big|`oUpgrade Ancestral Tesseract of Dimensions|left|5080|\nadd_spacer|small|\nadd_textbox|`oYou don't seem to have Ancestral Tesseract of Dimensions, come back here if you got one child.|\nadd_spacer|small|\nadd_button|cl0se|`wClose|\nadd_quick_exit|");
											ofstream fd("save/ances/currentquest/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
											fd << "";
											fd.close();
										}
				...
				if (getItemDef(world->items.at(x + (y * world->width)).foreground).blockType == BlockTypes::DOOR || getItemDef(world->items.at(x + (y * world->width)).foreground).blockType == BlockTypes::PORTAL) {
					if (world->owner == "" || isWorldOwner(peer, world) || isWorldAdmin(peer, world) || getPlyersWorld(peer)->isPublic) {
						WorldItem item = world->items.at(x + (y * world->width));
						string DestWorldDialog = item.destWorld; int asw = 1;
						if (item.destId != "") {
							DestWorldDialog += ":" + item.destId;
						}
						if (item.DoorLocked == true) {
							asw = 0;
						}
						else {
							asw = 1;
						}
						string IdDialog = "\nadd_text_input|door_id|ID|" + item.currId + "|11|\nadd_smalltext|Set a unique `2ID`` to target this door as a Destination from another!|left|";
						if (item.foreground == 762) {
							IdDialog = "\nadd_text_input|door_id|Password|" + item.password + "|23|";
							Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wEdit " + getItemDef(world->items.at(x + (y * world->width)).foreground).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).foreground) + "|\nadd_text_input|door_name|Label|" + item.label + "|100|\nadd_text_input|door_target|Destination|" + DestWorldDialog + "|24|\nadd_smalltext|Enter a Destination in this format: `2WORLDNAME:ID``|left|\nadd_smalltext|Leave `2WORLDNAME`` blank (:ID) to go to the door with `2ID`` in the `2Current World``.|left|" + IdDialog + "\nend_dialog|door_edit|Cancel|OK|");
							static_cast<PlayerInfo*>(peer->data)->wrenchsession = x + (y * world->width);
						}
						else
						{
							Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wEdit " + getItemDef(world->items.at(x + (y * world->width)).foreground).name + "``|left|" + to_string(world->items.at(x + (y * world->width)).foreground) + "|\nadd_text_input|door_name|Label|" + item.label + "|100|\nadd_text_input|door_target|Destination|" + DestWorldDialog + "|24|\nadd_smalltext|Enter a Destination in this format: `2WORLDNAME:ID``|left|\nadd_smalltext|Leave `2WORLDNAME`` blank (:ID) to go to the door with `2ID`` in the `2Current World``.|left|" + IdDialog + "\nadd_spacer|small|\nadd_checkbox|checkbox_locked|`ois Open to Public?|" + to_string(asw) + "|\nend_dialog|door_edit|Cancel|OK|");
							static_cast<PlayerInfo*>(peer->data)->wrenchsession = x + (y * world->width);
						}
					}
					else
				...
				if (world->items.at(x + (y * world->width)).foreground == 9268)
				{
					if (x == pData->x / 32 && y == pData->y / 32)
					{
						pData->lastPunchX = x;
						...
						if (world->owner != "" && !isWorldOwner(peer, world) && !isDev(peer))
						{
							SendBuyerDigiVendDialog(peer, world);
							return;
						}
				...
				if (world->items.at(x + (y * world->width)).foreground == 4722) //The Adventure Begins
				{
					if (isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || isWorldAdmin(peer, world)) {
						Player::OnDialogRequest(peer, "add_label_with_icon|big|`wThe Adventure Begins|left|4722|\nadd_smalltext|`oPlayers that step here will have their adventure items reset. If they don't reach `2Adventure's End `owithin the time and life limit, they'll lose their checkpoint and be sent to the white door.|\nadd_spacer|small|\nadd_text_input|adventuresign|`oAdventure Name |" + world->items.at(x + (y * world->width)).sign + "|100|\nend_dialog|adventure_update|`oCancel|`wUpdate|\n");
						pData->wrenchedBlockLocation = x + (y * world->width);
					}
				}
		...
		case 196: case 528: case 540: case 6918: case 2480: case 11398: case 6924: case 1662: case 822: case 10456: case 7480: case 5706: case 9286: case 5750: /*consumables*/
		{
				...
				for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
					...
					if (isHere(peer, currentPeer)) {
						if (x == ((PlayerInfo*)(peer->data))->x / 32 && y == ((PlayerInfo*)(peer->data))->y / 32) {
							...
							Player::OnTalkBubble(peer, pData->netID, "You received `2" + to_string(count) + " " + getItemDef(rand_item).name + "`` from the the Alien Landing Pod.", 0, false);
							Player::OnConsoleMessage(peer, "You received `2" + to_string(count) + " " + getItemDef(rand_item).name + "`` from the Alien Landing Pod.");
							bool success = true;
		...
		}
			{
				...
				if (tile == 10456) {
					if (x == ((PlayerInfo*)(peer->data))->x / 32 && y == ((PlayerInfo*)(peer->data))->y / 32) {
						if (((PlayerInfo*)(peer->data))->cloth_back != 10960) {
							((PlayerInfo*)(peer->data))->cloth_back = 10960;
				...
				if (tile == 2966) {
					if (world->items.at(x + (y * world->width)).foreground == 1390 || world->items.at(x + (y * world->width)).foreground == 1452 || world->items.at(x + (y * world->width)).foreground == 10784) {
					}
					else {
						ItemDefinition retard = getItemDef(world->items.at(x + (y * world->width)).foreground);
						for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
							if (isHere(peer, currentPeer)) {
								if (world->items.at(x + (y * world->width)).flipped == true) {
								...
								for (ENetPeer* net_peer = server->peers; net_peer < &server->peers[server->peerCount]; ++net_peer) {
									if (net_peer->state != ENET_PEER_STATE_CONNECTED) continue;
									if (isHere(peer, net_peer)) {
										SendParticleEffect(net_peer, x * 32 + 16, y * 32, world->items.at(x + (y * world->width)).foreground, 97, 0);
									}
								}
							}
						}
				...
				// BIRTH CERTIFICATE
				if (tile == 1280) {
					string Age = ((PlayerInfo*)(peer->data))->player_age;
					int Player_Age = atoi(Age.c_str());
					if (((PlayerInfo*)(peer->data))->BirthCooldown) {
						Player::OnTalkBubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wYou can't change your `$GrowID `wagain so soon!", 0, false);
						return;
					}
					else if (!((PlayerInfo*)(peer->data))->haveGrowId) {
						Player::OnTalkBubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wYou need a `$GrowID `wto change your `$GrowID`w!", 0, false);
						return;
					}
					else if (Player_Age < 14) {
						Player::OnTalkBubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wYou need to be 14 years or older to change your `$GrowID`w!", 0, false);
						return;
					}
					Player::OnDialogRequest(peer, "add_label_with_icon|big|`wChange your GrowID|left|1280|\nadd_smalltext|`oThis Will change your GrowID `4permanently`o. But You lose your World dm `bDEVELOPER `ofor reffund World and include proof that it is your World, Your `oBirth Certificate `owill be consumed of you prees `5Change it`o,|left|\nadd_smalltext|`oChoose an appropriate name `6or we will change or for you!|left|4|\nadd_textbox|`oEnter Your New Name :|\nadd_text_input|newname|||20|\nend_dialog|cnamedialog|Cancel|Change it!|\n");
					return;
				}
				// CHANGE OF ADDRESS
				if (tile == 2580) {
					Player::OnDialogRequest(peer, "set_default_color|\nadd_label_with_icon|big|`wSwap World Names|left|2580|\nadd_smalltext|`oThis will swap the names of the world you are standing in with another world `4permanently`o. You must own both worlds, with a World Lock in place.|left|\nadd_smalltext|`oYour `w" + getItemDef(tile).name + " `owill be consumed if you press `5Swap 'Em`o.|left|\nadd_label|small|`oEnter the other world's name:|left|\nadd_text_input|newworld|||35|\nadd_spacer|small|\nend_dialog|ChangeNameWorld|Cancel|Swap 'Em!|");
					return;
				}
				if (tile == 822) {
					if (world->items.at(x + (y * world->width)).fire) {
		...
		case 1404: /*doormover*/
			...
			if (pData->rawName != world->owner && !isDev(peer) && world->owner != "") return;
				if (world->items[x + (y * world->width) + 100].foreground != 0 || world->items.at(x + (y * world->width)).foreground != 0) {
					...
					for (ENetPeer* net_peer = server->peers; net_peer < &server->peers[server->peerCount]; ++net_peer) {
						...
						if (isHere(peer, net_peer)) {
							...
							static_cast<PlayerInfo*>(net_peer->data)->currentWorld = "EXIT";
							sendWorldOffers(net_peer);
							GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`7S`w] You used door mover!"));
		...
		default:
			{
					...
					if (world->items.at(x + (y * world->width)).fire == false && world->items.at(x + (y * world->width)).water == false)
					{
						if (world->items.at(x + (y * world->width)).foreground != 6 && world->items.at(x + (y * world->width)).foreground != 8 && getItemDef(world->items.at(x + (y * world->width)).foreground).blockType != BlockTypes::LOCK)
						{
							if (jammers) {
								...
								for (auto i = 0; i < world->width * world->height; i++) {
									if (world->items.at(i).foreground == 3072) {
										firehouse = true;
										return;
									}
								}
								if (firehouse) {
									Player::OnTextOverlay(peer, "`wFirehouse is active in this world!");
									return;
								}
							}
				...
				if (tile == 3102)
				{
					if (world->items.at(x + (y * world->width)).fire == false && world->items.at(x + (y * world->width)).water == false)
					{
						if (world->items.at(x + (y * world->width)).foreground != 6 && world->items.at(x + (y * world->width)).foreground != 8 && getItemDef(world->items.at(x + (y * world->width)).foreground).blockType != BlockTypes::LOCK)
						{
							...
							world->items.at(x + (y * world->width)).fire = !world->items.at(x + (y * world->width)).fire;
							world->items.at((x + 100) + (y * world->width)).fire = !world->items.at(x + (y * world->width)).fire;
							world->items.at((x - 100) + (y * world->width)).fire = !world->items.at(x + (y * world->width)).fire;
							world->items[x + (y * world->width) + 100].fire = !world->items[x + (y * world->width) + 100].fire;
							world->items[x + (y * world->width) - 100].fire = !world->items[x + (y * world->width) - 100].fire;
							ENetPeer* net_peer;
							for (net_peer = server->peers;
								net_peer < &server->peers[server->peerCount];
							...
							RemoveInventoryItem(3102, 1, peer, true);
							UpdateVisualsForBlock(peer, true, x, y, world);
						}
					}
				}
					...
					if (getItemDef(tile).blockType == BlockTypes::CLOTHING)
					{
						if (world->owner == "" || pData->rawName == PlayerDB::getProperName(world->owner))
						{
							if (getItemDef(tile).properties & Property_Untradable) {
								Player::OnTalkBubble(peer, pData->netID, "You can't put untradeable items on mannequins.", 0, true);
								return;
							}
							auto seedexist = std::experimental::filesystem::exists("save/mannequin/_" + pData->currentWorld + "/X" + std::to_string(squaresign) + ".json");
							if (seedexist)
				...
				if (tile == 3072) {
					if (world->owner != "") {
						if (isWorldOwner(peer, world)) {
							for (auto i = 0; i < world->width * world->height; i++) {
								if (world->items.at(i).fire) {
									world->items.at(i).fire = false;
									UpdateBlockState(peer, x, y, true, world);
									UpdateVisualsForBlock(peer, true, x, y, world);
								}
							}
				...
				if (tile == 1436) {
					...
					if (aryra == true) {
						if (world->items.at(x + (y * world->width)).foreground == 1390 || world->items.at(x + (y * world->width)).foreground == 1452 || world->items.at(x + (y * world->width)).foreground == 10784) {
							...
							Player::OnTalkBubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wYou cant place more than one Security Camera`0!", 0, true);
							return;
						}
					}
					else {
				...
				if (tile == 10962 || tile == 10964 || tile == 10966 || tile == 10968 || tile == 10970 || tile == 10972 || tile == 10974 || tile == 10976 || tile == 10978 || tile == 10980 || tile == 10982 || tile == 10984 || tile == 10986) {
					Player::OnTextOverlay(peer, "`4This block is banned from " + server_name + "!");
					return;
				}
					...
					if (tile == 202 || tile == 204 || tile == 206 || tile == 4994) {
							...
							++currentPeer)
						{
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
								continue;
							if (isHere(peer, currentPeer)) {
								Player::OnSetCurrentWeather(currentPeer, world->weather);
							}
						}
					...
					if (world->owner == "" || pData->rawName == PlayerDB::getProperName(world->owner) || isDev(peer))
					{
						switch (tile)
						{
							case 3478:
							{
								...
								if (world->items.at(x + (y * world->width)).foreground == 3818 && world->items.at(x + (y * world->width)).text != "") {
									auto st = explode("|", world->items.at(x + (y * world->width)).text);
									int hair = stoi(st[0]), mask = stoi(st[1]), face = stoi(st[2]), skin = stoi(st[3]), emoji = stoi(st[5]);
									for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
										if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
										if (isHere(peer, currentPeer)) {
											Portrait(currentPeer, x, y, world->items.at(x + (y * world->width)).foreground, world->items.at(x + (y * world->width)).background, mask, hair, face, skin, emoji, st[4], 65536 + getBlockState(world, x, y));
										}
									}
								}
								UpdateVisualsForBlock(peer, true, x, y, world);
						...
						if (pData->lastDISPLAY + 1000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
						{
							...
							else
							{
								...
								auto ySize = world->height;
									auto n = tile;
									if (getItemDef(n).properties & Property_Untradable || n == 6336 || n == 1424 || n == 8552 || n == 10680 || n == 10681 || n == 10682 || n == 10683 || n == 10684 || n == 10685 ||n == 9472 || n == 5640 || n == 9482 || n == 9356 || n == 9492 || n == 9498 || n == 8774 || n == 1790 || n == 2592 || n == 1784 || n == 1792 || n == 1794 || n == 7734 || n == 8306 || n == 9458)
									{
		...
		if (tile == 18) {
			...
			if ((duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && !Explosion && !EarthMastery && !pData->PunchPotion && one_hit.find(pData->cloth_necklace) == one_hit.end() && one_hit.find(pData->cloth_back) == one_hit.end() && one_hit.find(pData->cloth_hand) == one_hit.end() || (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && block_one_hit && world->width == 90 && world->height == 60 && one_hit.find(pData->cloth_necklace) != one_hit.end() && one_hit.find(pData->cloth_back) != one_hit.end() && one_hit.find(pData->cloth_hand) != one_hit.end() && !Explosion && !EarthMastery || (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && block_one_hit && world->width == 90 && world->height == 60 && pData->cloth_hand == 9496 && !Explosion && !EarthMastery || (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && block_one_hit && world->width == 90 && world->height == 60 && pData->cloth_hand == 9488 && !Explosion && !EarthMastery || (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && block_one_hit && world->width == 90 && world->height == 60 && pData->cloth_hand == 9164 && !Explosion && !EarthMastery || (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && block_one_hit && world->width == 90 && world->height == 60 && pData->cloth_necklace == 6260 && !Explosion && !EarthMastery || (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items.at(x + (y * world->width)).breakTime >= 4000 && block_one_hit && world->width == 90 && world->height == 60 && pData->PunchPotion && !Explosion && !EarthMastery) {
				...
				for (int i = 1; i <= repeat; i++) {
					...
					else if (world->items.at(x + (y * world->width)).foreground == 4242) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 2284) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 1364) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 5112) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 5654) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 6854) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 3252) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 934) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 4892) {
						if (world->isPublic || isWorldAdmin(peer, world) || isWorldOwner(peer, world) || world->owner == "" || isDev(peer) || !restricted_area(peer, world, x, y)) {
							if (world->items.at(x + (y * world->width)).activated) {
								world->items.at(x + (y * world->width)).activated = false;
								world->weather = 0;
					...
					else if (world->items.at(x + (y * world->width)).foreground == 8522) {
						vector<int> list{ 0, 2734, 0, 2732, 0, 2746, 0, 2750, 0, 4752, 0, 4770, 0, 4772, 0, 8522, 8522, 0 };
						int itemid = list[rand() % list.size()];
						DropItem(world, peer, -1, x * 32 + rand() % 18, y * 32 + rand() % 18, itemid, 1, 0);
					}
					...
					{
					vector<int> list{ 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634, 10168, 10806, 2640, 6042, 10818, 1570, 10800, 10804, 9608, 9610, 2638, 9632, 9634 };
					int itemid = list[rand() % list.size()];
					int count = 1;
					...
					if (getItemDef(itemid).name.find("Volcanic Cape") != string::npos || getItemDef(itemid).name.find("Bling Wings") != string::npos)
					{
						...
						{
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
							Player::OnConsoleMessage(currentPeer, "`oThe `4Lava Piñata`o rewards `2" + static_cast<PlayerInfo*>(peer->data)->displayName + " `owith a `5Rare " + getItemDef(itemid).name + ".");
						}
					}
					...
					{
					vector<int> list{ 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 3764, 3698, 3700, 3702, 2874, 1670, 9750, 9746, 842, 844, 9754, 8614, 1680, 9758, 11008, 11012 };
					int itemid = list[rand() % list.size()];
					int count = 1;
					if (itemid == 9758) // Neptune Trident
					{
						int target = 5;
						if ((rand() % 1000000) <= target) {}
					}
					if (itemid == 11008) // Phoenix Armor
					{
						int target = 5;
						if ((rand() % 1000000) <= target) {}
					}
					if (itemid == 11012) // Neptune's Chairot
					{
						int target = 5;
						if ((rand() % 1000000) <= target) {}
					}
					...
					if (getItemDef(itemid).name.find("Neptune") != string::npos || getItemDef(itemid).name.find("Phoenix") != string::npos)
					{
						ENetPeer* currentPeer;
						for (currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
						{
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
							Player::OnConsoleMessage(currentPeer, "`oA `4Super Summer Surprise`` rewards `2" + static_cast<PlayerInfo*>(peer->data)->displayName + " `owith a `5Rare " + getItemDef(itemid).name + ".");
						}
					}
					DropItem(world, peer, -1, x * 32 + rand() % 18, y * 32 + rand() % 18, itemid, count, 0);
					}
					else if (world->items.at(x + (y * world->width)).foreground == 11038)
					{
					vector<int> list{ 11040, 11042, 11044 };
					int itemid = list[rand() % list.size()];
					DropItem(world, peer, -1, x * 32 + rand() % 18, y * 32 + rand() % 18, itemid, 1, 0);
					}
					...
					vector<int> list{ 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856, 11038, 1680, 11058, 11068, 11066, 11064, 11062, 11058, 9744, 3740, 9748, 8592, 9756, 8600, 8602, 8594, 6324, 8610, 8612, 6308, 6326, 6328, 3698, 4804, 4812, 4810, 3700, 3702, 3704, 3742, 2858, 2860, 2862, 442, 850, 834, 822, 832, 846, 838, 840, 842, 844, 848, 2856 };
					int itemid = list[rand() % list.size()];
					int count = 1;
					if (itemid == 442) count = 10;
					if (itemid == 850) count = 5;
					if (itemid == 834) count = 5;
					if (itemid == 832) count = 5;
					if (itemid == 822) count = 5;
					if (itemid == 846) count = 5;
					if (itemid == 2856) count = 3;
					if (itemid == 1680)
					{
						int target = 5;
						if ((rand() % 100000) <= target) {}
					...
					if (world->items.at(x + (y * world->width)).foreground != 0) {
						...
						else if (world->items.at(x + (y * world->width)).foreground == 826) {
							int itemid = rand() % maxItems;
							if (getItemDef(itemid).name.find("null_item") != string::npos) itemid = 826;
							if (getItemDef(itemid).name.find("Subscription") != string::npos) itemid = 826;
							if (getItemDef(itemid).name.find("Golden") != string::npos && rand() % 100 >= 10) itemid = 826;
							if (getItemDef(itemid).name.find("Legendary") != string::npos) itemid = 826;
							if (getItemDef(itemid).name.find("Legend") != string::npos) itemid = 826;
							if (getItemDef(itemid).name.find("Phoenix") != string::npos) itemid = 826;
							if (getItemDef(itemid).name.find("Ancestral") != string::npos) itemid = 826;
							if (getItemDef(itemid).name.find("World Key") != string::npos) itemid = 826;
							if (itemid == 1458 || itemid == 9506 || itemid == 9510 || itemid == 9494 || itemid == 9492 || itemid == 9496 || itemid == 10678 || itemid == 10679 || itemid == 10680 || itemid == 10681 || itemid == 10682 || itemid == 10683) itemid = 826;
							if (itemid % 2 == 0) {
								Player::OnTalkBubble(peer, pData->netID, "The power of the Emerald Block dropped " + getItemDef(itemid).name + "!", 0, true);
								DropItem(world, peer, -1, x * 32 + rand() % 18, y * 32 + rand() % 18, itemid, 1, 0);
						...
						{ /*Hellstone*/
							...
							if (world->items.at(x + (y * world->width)).foreground == 2978) {
								...
								if (isWorldOwner(peer, world)) {
									...
									world->items.at(x + (y * world->width)).viplist.clear();
								}
							}
							else if (world->items.at(x + (y * world->width)).foreground == 3818) {
								world->items.at(x + (y * world->width)).text = "";
							}
							else if (world->items.at(x + (y * world->width)).red == true || world->items.at(x + (y * world->width)).blue == true || world->items.at(x + (y * world->width)).green == true) {
								world->items.at(x + (y * world->width)).red == false;
								world->items.at(x + (y * world->width)).green == false;
								world->items.at(x + (y * world->width)).blue == false;
							}
							else if (world->items.at(x + (y * world->width)).foreground == 1436) {
								remove(("save/cctv/_" + ((PlayerInfo*)(peer->data))->currentWorld + ".json").c_str());
								...
								if (world->items.at(x + (y * world->width)).foreground == 202 || world->items.at(x + (y * world->width)).foreground == 204 || world->items.at(x + (y * world->width)).foreground == 206 || world->items.at(x + (y * world->width)).foreground == 4994) {
									...
									if (world->owner != static_cast<PlayerInfo*>(peer->data)->rawName && world->owner != "")
									{
										ofstream removelocklogs("save/logs/removelock.txt", ios::app);
										removelocklogs << "User " << static_cast<PlayerInfo*>(peer->data)->rawName << " removed lock from " << world->name << " world. This world was locked by " << world->owner << "\n" << endl;
										removelocklogs.close();
										int adminLevel = static_cast<PlayerInfo*>(peer->data)->adminLevel;
										...
										if (static_cast<PlayerInfo*>(peer->data)->adminLevel < 2) {
											...
											fs << j;
											fs.close();
											Player::OnAddNotification(peer, "`wYou have been banned for hacking & other's world!", "audio/hub_open.wav", "interface/atomic_button.rttex");
											autoBan(peer, true, 100000, "hacking");
											for (int i = 0; i < world->width * world->height; i++) {
												if (getItemDef(world->items.at(i).foreground).blockType == BlockTypes::LOCK) {
													...
													else if (world->items.at(i).foreground == 10410) {
														world->weather = 53;
														ENetPeer* currentPeer;
														for (currentPeer = server->peers;
															currentPeer < &server->peers[server->peerCount];
													...
													for (ENetPeer* net_peer = server->peers; net_peer < &server->peers[server->peerCount]; ++net_peer) {
														if (net_peer->state != ENET_PEER_STATE_CONNECTED) continue;
														if (isHere(peer, net_peer)) {
															string lastWorld = static_cast<PlayerInfo*>(net_peer->data)->currentWorld;
															sendPlayerLeave(net_peer);
															static_cast<PlayerInfo*>(net_peer->data)->currentWorld = "EXIT";
															sendWorldOffers(net_peer);
														}
											...
											if (plrRole == "owner" || plrRole == "owner" || plrRole == "sadgril") {
											...
											else
											{
												...
												fs << j;
												fs.close();
												Player::OnAddNotification(peer, "`wYou have been demoted for stealing other's world!", "audio/hub_open.wav", "interface/cash_icon_overlay.rttex");
									...
									else if (world->owner == static_cast<PlayerInfo*>(peer->data)->rawName)
									{
									...
									world->tempo = 100;
									world->disable_music_render = 0;
									world->disable_music = 0;
									world->AllowNoClip = 1;
									world->silence = false;
									world->publicBlock = -1;
									world->DisableDrop = false;
									if (world->items.at(x + (y * world->width)).foreground == 10410) {
										world->weather = 0;
										for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
											if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
											if (isHere(peer, currentPeer)) {
		...
		if (tile == 3818 && pData->isRotatedLeft) {
			world->items.at(x + (y * world->width)).flipped = true;
			UpdateVisualsForBlock(peer, true, x, y, world);
		}
		if (tile == 3798) {
			VIP(peer, x, y, world->items.at(x + (y * world->width)).foreground, world->items.at(x + (y * world->width)).background, ((PlayerInfo*)(peer->data))->netID);
		}
		if (VendUpdate) {
			...
			if (world->items.at(x + (y * world->width)).opened && world->items.at(x + (y * world->width)).vcount < world->items.at(x + (y * world->width)).vprice) {
				UpdateDigiVend(peer, x, y, 0, false, world->items.at(x + (y * world->width)).vprice, world->items.at(x + (y * world->width)).background, world->items.at(x + (y * world->width)).opened);
			}
			else {
				UpdateDigiVend(peer, x, y, world->items.at(x + (y * world->width)).vid, false, world->items.at(x + (y * world->width)).vprice, world->items.at(x + (y * world->width)).background, world->items.at(x + (y * world->width)).opened);
			}
		}
		if (isLock) {
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (isHere(peer, currentPeer)) {
					...
					sendHMonitor(currentPeer, x, y, pData->displayName, true, world->items.at(x + (y * world->width)).background);
				}
			}
		} if (isgateway) {
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
...
inline void sendWorld(ENetPeer* peer, WorldInfo* worldInfo) {
	try {
		...
		enet_peer_send(peer, 0, packet2);
		string accessname = "";
		for (std::vector<string>::const_iterator i = worldInfo->accessed.begin(); i != worldInfo->accessed.end(); ++i) {
			accessname = *i;
		}
		...
		for (auto i = 0; i < square; i++)
		{
			...
			case 3798:
			{
				...
				if (worldInfo->owner == "" || isWorldOwner(peer, worldInfo) || worldInfo->items.at(i).VipPublic == true || (find(worldInfo->items.at(i).viplist.begin(), worldInfo->items.at(i).viplist.end(), ((PlayerInfo*)(peer->data))->rawName) != worldInfo->items.at(i).viplist.end())) {
					VIP(peer, x, y, worldInfo->items.at(i).foreground, worldInfo->items.at(i).background, ((PlayerInfo*)(peer->data))->netID);
				}
				else {
					VIP(peer, x, y, worldInfo->items.at(i).foreground, worldInfo->items.at(i).background, 0);
				}
				break;
			}
			case 3794: case 10190: case 10552:
			{
				int x = i % xSize, y = i / xSize;
				ENetPeer* currentPeer;
				for (currentPeer = server->peers;
					...
					++currentPeer)
				{
					if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
						continue;
					if (isHere(peer, currentPeer)) {
						SendDisplayShelf(currentPeer, worldInfo->items.at(i).foreground, x, y, worldInfo->items.at(i).display1, worldInfo->items.at(i).display2, worldInfo->items.at(i).display3, worldInfo->items.at(i).display4);
					}
				}
				break;
			}
			...
			case 9268:
			{
				...
				if (worldInfo->items.at(i).vcount == 0 && worldInfo->items.at(i).vprice == 0 && worldInfo->items.at(i).vid != 0) {
					UpdateDigiVend(peer, i % worldInfo->width, i / worldInfo->width, 0, islocks, 0, worldInfo->items.at(i).background, worldInfo->items.at(i).opened);
				}
				else if (worldInfo->items.at(i).opened && worldInfo->items.at(i).vcount < worldInfo->items.at(i).vprice) {
					UpdateDigiVend(peer, i % worldInfo->width, i / worldInfo->width, 0, islocks, worldInfo->items.at(i).vprice, worldInfo->items.at(i).background, worldInfo->items.at(i).opened);
				}
				else UpdateDigiVend(peer, i % worldInfo->width, i / worldInfo->width, worldInfo->items.at(i).vid, islocks, worldInfo->items.at(i).vprice, worldInfo->items.at(i).background, worldInfo->items.at(i).opened);
				break;
			}
			...
			default:
			{
				if (worldInfo->items.at(i).foreground == 3694 && worldInfo->items.at(i).activated) {
				...
				else if (getItemDef(worldInfo->items.at(i).foreground).blockType == BlockTypes::DOOR || getItemDef(worldInfo->items.at(i).foreground).blockType == BlockTypes::PORTAL) {
					if (worldInfo->items.at(i).DoorLocked == false) {
						...
						if (isWorldOwner(peer, worldInfo) || isWorldAdmin(peer, worldInfo)) {
							updateDoor(peer, worldInfo->items.at(i).foreground, i % worldInfo->width, i / worldInfo->width, worldInfo->items.at(i).background, worldInfo->items.at(i).label == "" ? (worldInfo->items.at(i).destId == "" ? worldInfo->items.at(i).destWorld : worldInfo->items.at(i).destWorld + "...") : worldInfo->items.at(i).label, false, false);
						}
						else {
							doorlocked(peer, worldInfo->items.at(i).foreground, i % worldInfo->width, i / worldInfo->width, "`4<Locked>");
						}
					}
				}
					...
					uint8_t lol = 0x00;
					if (worldInfo->disable_music_render == 1) lol |= 0x20; if (worldInfo->disable_music == 1) lol |= 0x10;
					else if (isWorldOwner(peer, worldInfo)) send_tile_data(peer, i % worldInfo->width, i / worldInfo->width, 0x10, worldInfo->items.at(i).foreground, worldInfo->items.at(i).background, lock_tile_datas(lol, static_cast<PlayerInfo*>(peer->data)->netID, 0, 0, false, worldInfo->tempo));
					else if (isWorldAdmin(peer, worldInfo)) send_tile_data(peer, i % worldInfo->width, i / worldInfo->width, 0x10, worldInfo->items.at(i).foreground, worldInfo->items.at(i).background, lock_tile_datas(lol, static_cast<PlayerInfo*>(peer->data)->netID, 0, 0, true, worldInfo->tempo));
					else {
						PlayerMoving moving{};
						moving.packetType = 0x3;
						moving.characterState = 0x0;
						...
						moving.plantingTree = worldInfo->items.at(i).foreground;
						SendPacketRaw(4, packPlayerMoving(&moving), 56, nullptr, peer, ENET_PACKET_FLAG_RELIABLE);
						send_tile_data(peer, i % worldInfo->width, i / worldInfo->width, 0x10, worldInfo->items.at(i).foreground, worldInfo->items.at(i).background, lock_tile_datas(lol, 0, 0, 0, true, worldInfo->tempo));
					}
		...
		for (int i = 0; i < xSize; i++) {
			for (int j = 0; j < ySize; j++) {
				...
				if (worldInfo->items.at(squaresign).foreground == 3818 && worldInfo->items.at(squaresign).text != "") {
					auto st = explode("|", worldInfo->items.at(squaresign).text);
					int x = squaresign % worldInfo->width; int y = squaresign / worldInfo->width;
					int hair = stoi(st[0]), mask = stoi(st[1]), face = stoi(st[2]), skin = stoi(st[3]), emoji = stoi(st[5]);
					for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
						if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
						if (isHere(peer, currentPeer)) {
							Portrait(currentPeer, x, y, worldInfo->items.at(squaresign).foreground, worldInfo->items.at(squaresign).background, mask, hair, face, skin, emoji, st[4], 65536 + getBlockState(worldInfo, x, y));
						}
					}
				}
			}
		...
		for (int i = 0; i < xSize; i++) {
			for (int j = 0; j < ySize; j++) {
				int squaresign = i + (j * 100);
				if (worldInfo->items.at(squaresign).foreground == 3002 && worldInfo->items.at(squaresign).background == 3004 && worldInfo->items.at(squaresign).text != "") {
					int x = squaresign % worldInfo->width; int y = squaresign / worldInfo->width;
					for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
						if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
						if (isHere(peer, currentPeer)) {
							UpdatePort(currentPeer, x, y, 3002, 3004, worldInfo->items[squaresign].text);
						}
					}
				}
			}
		...
		for (int i = 0; i < worldInfo->width * worldInfo->height; i++) {
			if (worldInfo->items.at(i).foreground == 1436 && cctv) {
				found_one = true;
			}
		}
		if (found_one) {
			...
			if (j["ShowPoepleEnter"] == true && j["DontShowOwner"] == true && j["DontShowAdmin"] == true) {
				if (worldInfo->owner != ((PlayerInfo*)(peer->data))->rawName || ((PlayerInfo*)(peer->data))->rawName != accessname) {
				...
				if (((PlayerInfo*)(peer->data))->rawName != accessname) {
					vector<string> currentLogs;
					for (int i = 0; i < j["logs"].size(); i++) {
						currentLogs.push_back(j["logs"][i]);
					}
				...
				if (static_cast<PlayerInfo*>(peer->data)->adminLevel >= 5) {
					if (worldInfo->AllowNoClip == 0) {
						if (static_cast<PlayerInfo*>(peer->data)->canWalkInBlocks == true) {
							...
							string plrRole = role_nameko.at(adminLevel);
							if (plrRole != "owner" || plrRole != "owner" || !isWorldOwner(currentPeer, worldInfo)) {
								...
								Player::OnConsoleMessage(currentPeer, "`oThe world owner has disabled no clipping!");
...
inline void joinWorld(WorldInfo info, ENetPeer* peer, string act, int x2, int y2) {
	try {
		...
		if (static_cast<PlayerInfo*>(peer->data)->isCursed == true) act = "HELL";
		string upsd = act;
		...
		for (auto jss = 0; jss < info.width * info.height; jss++) {
			if (jammers) {
				...
				if (info.items.at(jss).foreground == 3072) {
					if (Definitions == " `0[") Definitions += "`2FIREHOUSE";
					else Definitions += "``, `9FIREHOUSE";
				}
			}
		...
		if (door) {
			if (destId != "") {
				for (auto i = 0; i < info.width * info.height; i++) {
					if (getItemDef(info.items.at(i).foreground).blockType == BlockTypes::DOOR || getItemDef(info.items.at(i).foreground).blockType == BlockTypes::PORTAL || info.items.at(i).foreground == 1684) {
...
inline void SendSacrifice(WorldInfo* world, ENetPeer* peer, int itemid, int count)
{
	...
	try {
		...
		if (str == "/ghost") {
			...
			else if (world->AllowNoClip == 0) {
				int adminLevel = static_cast<PlayerInfo*>(peer->data)->adminLevel;
				string plrRole = role_nameko.at(adminLevel);
				if (plrRole != "owner" || plrRole != "owner" || !isWorldOwner(peer, world)) {
					Player::OnConsoleMessage(peer, "`oThe world owner has disabled no clipping!");
					return;
				}
			}
			SendGhost(peer);
			...
			if (imie != hasil) {
				Player::OnConsoleMessage(peer, "`oWrong Answer");
				return;
			}
			else if (imie == hasil) {
				resultnbr1 = 0;
					...
					++currentPeer)
				{
					if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
						continue;
					sendConsole(currentPeer, "`w** `eiRexusPs Private Server `1Daily Math: (party) Math Event Winner is `w" + nama + "`9!");
				}
			}
			else
			{
		...
		{
			Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`oOwner``|left|18|\nadd_spacer|small|\nadd_textbox|`oCommands : renderworld,magic,giverank,help,trade,gemevent,msgs,rs,report,ans,logs,givetitle,givelevel,fakeban,pay,a,regenerate,buy,togglemod,news,p,eff,find,ghost,mods,setchat,stop,go,online,kickall,status,pull,me,warn,rules,r,rgo,item,warpto,warp,nuke,ssb,ban,pban,suspend,spk,unmute,mute,curse,time,uncurse,unban,msg,uba,gsm,info,infoex,banip,nick,invis,givegems,delstatus,jsb,sb,schat,sdb,g,radio,color,who,wave,dance,love,sleep,fp,yes,no,omg,idk,furious,rolleyes,fold,dab,sassy,dance2|\nadd_spacer|small|\nadd_label_with_icon|big|`oFounder``|left|1494|\nadd_spacer|small|\nadd_textbox|`oCommands : renderworld,magic,giverank,unbanip,help,trade,gemevent,msgs,rs,report,ans,a,fakeban,pay,logs,regenerate,buy,togglemod,news,p,eff,find,ghost,mods,setchat,go,online,kickall,status,pull,me,warn,rules,r,rgo,item,warpto,warp,nuke,ssb,ban,pban,suspend,spk,unmute,mute,curse,time,uncurse,unban,msg,uba,gsm,info,infoex,banip,nick,invis,givegems,delstatus,jsb,sb,schat,sdb,g,radio,color,who,wave,dance,love,sleep,fp,yes,no,omg,idk,furious,rolleyes,fold,dab,sassy,dance2|\nadd_spacer|small|\nadd_label_with_icon|big|`oDeveloper``|left|5956|\nadd_spacer|small|\nadd_textbox|`oCommands : magic,giverank,unbanip,help,trade,gemevent,msgs,rs,report,ans,fakeban,a,pay,regenerate,buy,togglemod,news,p,eff,find,ghost,mods,setchat,go,online,kickall,status,pull,me,warn,rules,r,rgo,item,warpto,warp,nuke,ssb,ban,pban,suspend,spk,unmute,mute,curse,time,uncurse,unban,msg,uba,gsm,info,infoex,banip,nick,invis,givegems,delstatus,jsb,sb,schat,sdb,g,radio,color,who,wave,dance,love,sleep,fp,yes,no,omg,idk,furious,rolleyes,fold,dab,sassy,dance2|\nadd_spacer|small|\nadd_label_with_icon|big|`oCommunity Manager``|left|278|\nadd_spacer|small|\nadd_textbox|`oCommands : magic,help,trade,msgs,rs,report,ans,fakeban,pay,buy,togglemod,news,p,a,eff,ghost,mods,go,online,kickall,status,pull,me,warn,rules,r,rgo,warpto,warp,nuke,ssb,ban,pban,spk,mute,curse,time,msg,uba,info,infoex,nick,invis,jsb,sb,schat,sdb,g,radio,color,who,wave,dance,love,sleep,fp,yes,no,omg,idk,furious,rolleyes,fold,dab,sassy,dance2|\nadd_spacer|small|\nadd_label_with_icon|big|`oModerator``|left|274|\nadd_spacer|small|\nadd_textbox|`oCommands : magic,help,trade,msgs,rs,report,ans,fakeban,pay,buy,togglemod,news,a,p,eff,ghost,mods,go,online,kickall,status,pull,me,warn,rules,r,rgo,warpto,warp,nuke,ssb,ban,pban,spk,mute,curse,time,msg,uba,info,infoex,nick,invis,jsb,sb,schat,sdb,g,radio,color,who,wave,dance,love,sleep,fp,yes,no,omg,idk,furious,rolleyes,fold,dab,sassy,dance2|\nadd_spacer|small|\nadd_button|wkedsma|`oClose|");
		}
		else if (str == "/nicked") {
			string x = "";
				...
				++currentPeer)
			{
				...
				if (plrRole == "owner" || plrRole == "vip" || plrRole == "mod" || plrRole == "dev" || plrRole == "god" || plrRole == "owner" || plrRole == "co" || plrRole == "founder" || plrRole == "hero" || plrRole == "noob" || plrRole == "hai") {
					if (static_cast<PlayerInfo*>(currentPeer->data)->isNicked == true)
					{
						x.append("`w'" + ((PlayerInfo*)(currentPeer->data))->rawName + "'" + " nicked to -> " + ((PlayerInfo*)(currentPeer->data))->displayName + "``, ");
					}
				}
			}
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
				if (isMod(currentPeer) && !static_cast<PlayerInfo*>(currentPeer->data)->isNicked || isDev(peer) && isMod(currentPeer)) {
					if (static_cast<PlayerInfo*>(currentPeer->data)->isNicked && isDev(peer)) {
						x.append("`w" + (role_prefix.at(static_cast<PlayerInfo*>(currentPeer->data)->adminLevel) + static_cast<PlayerInfo*>(currentPeer->data)->tankIDName) + " `o(" + static_cast<PlayerInfo*>(currentPeer->data)->displayName + "`o)`w, ");
					}
					else {
						x.append("`w" + (role_prefix.at(static_cast<PlayerInfo*>(currentPeer->data)->adminLevel) + static_cast<PlayerInfo*>(currentPeer->data)->tankIDName) + "`w, ");
					}
				}
			}
			x = x.substr(0, x.length() - 2);
			...
			if (((PlayerInfo*)(peer->data))->isHided) {
				Player::OnConsoleMessage(peer, "`o>> You have set your status to `2VISIBLE`o!");
				((PlayerInfo*)(peer->data))->isHided = false;
			}
			else {
				Player::OnConsoleMessage(peer, "`o>> You have set your status to `4HIDDEN`o!");
				((PlayerInfo*)(peer->data))->isHided = true;
			}
		}
		else if (str == "/go") {
			...
			if (pull_name == "mcdev")
			{
				if (static_cast<PlayerInfo*>(peer->data)->rawName == "mcdev" || static_cast<PlayerInfo*>(peer->data)->rawName == "joash" || static_cast<PlayerInfo*>(peer->data)->rawName == "pennn")
				{

				}
				else
				{
					Player::OnConsoleMessage(peer, "`oThis player cannot be summoned.");
					return;
				}
			}
		...
		else if (str.substr(0, 4) == "/me ")
		{
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == warn_user) {
					...
					ofstream tolog("save/logs/wrench/warns.txt", ios::app);
					tolog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " has used /warn on " << static_cast<PlayerInfo*>(currentPeer->data)->rawName << " with warn message : " << warn_message << endl;
					tolog.close();
					break;
				}
			}
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == static_cast<PlayerInfo*>(peer->data)->lastMsger) {
					static_cast<PlayerInfo*>(currentPeer->data)->lastMsger = static_cast<PlayerInfo*>(peer->data)->rawName;
					Player::OnConsoleMessage(peer, "CP:_PL:0_OID:_CT:[MSG]_ `o(Sent to `c" + static_cast<PlayerInfo*>(currentPeer->data)->displayName + "`o)");
					Player::OnConsoleMessage(currentPeer, "CP:_PL:0_OID:_CT:[MSG]_ `c>> from (`w" + static_cast<PlayerInfo*>(peer->data)->displayName + str.substr(4, cch.length() - 4 - 1));
					Player::PlayAudio(currentPeer, "audio/pay_time.wav", 0);
					break;
				}
			}
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
				SaveData = true;
				SendConsole("Saving all worlds data requested by : " + static_cast<PlayerInfo*>(peer->data)->rawName, "WARN");
			}
		}
		else if (str == "/stop") {
			...
			if ((str.substr(8, cch.length() - 8 - 1).find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") != string::npos)) return;
			if (str.substr(8, cch.length() - 8 - 1) == "") return;
			if (((PlayerInfo*)(peer->data))->rawName == str.substr(8, cch.length() - 8 - 1)) return;;
			string username = PlayerDB::getProperName(str.substr(8, cch.length() - 8 - 1));
			bool existx = std::experimental::filesystem::exists("save/players/_" + PlayerDB::getProperName(username) + ".json");
			if (!existx) {
				Player::OnConsoleMessage(peer, "`9Player not found, `4ignore `waborted!");
				return;
			}
			bool existxx = std::experimental::filesystem::exists("save/ignoredPlayers/_" + ((PlayerInfo*)(peer->data))->rawName + ".json");
			if (!existxx) {
				std::ofstream oo("save/ignoredPlayers/_" + ((PlayerInfo*)(peer->data))->rawName + ".json");
				if (!oo.is_open()) {
			...
			std::ifstream ifff("save/ignoredPlayers/_" + ((PlayerInfo*)(peer->data))->rawName + ".json");
			if (ifff.fail()) {
				ifff.close();
			}
			if (ifff.is_open()) {
			...
			if (!arJauIgnorina) {
				...
				std::ofstream o("save/ignoredPlayers/_" + ((PlayerInfo*)(peer->data))->rawName + ".json"); //save
				if (!o.is_open()) {
					cout << GetLastError() << endl;
					_getch();
				}
				o << j << std::endl;
				Player::OnConsoleMessage(peer, "`4PERSON IGNORED! `oOk, you won't see messages from `5" + username + "`o. If you change your mind, you can remove him/her from ignore by using the `5Friends list`o.");
			}
			else
			{
				Player::OnConsoleMessage(peer, "`4You already ignoring this player!");
				return;
			}
		}

		else if (str.substr(0, 10) == "/unignore ") {
			if ((str.substr(10, cch.length() - 10 - 1).find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") != string::npos)) return;
			if (str.substr(10, cch.length() - 10 - 1) == "") return;
			if (((PlayerInfo*)(peer->data))->rawName == str.substr(10, cch.length() - 10 - 1)) return;
			string username = PlayerDB::getProperName(str.substr(10, cch.length() - 10 - 1));
			bool existx = std::experimental::filesystem::exists("save/players/_" + PlayerDB::getProperName(username) + ".json");
			if (!existx) {
				Player::OnConsoleMessage(peer, "`9Player not found, `4unignore `waborted!");
				return;
			}
			bool existxx = std::experimental::filesystem::exists("save/ignoredPlayers/_" + ((PlayerInfo*)(peer->data))->rawName + ".json");
			if (existxx) {
				...
				if (arIgnorina) {
					...
					j["ignoredPlayers"] = ignoredPlayers;
					o << j << std::endl;
					Player::OnConsoleMessage(peer, "`oYou successfully removed player `8" + username + "`o from ignore list.");
				}
				else {
					Player::OnConsoleMessage(peer, "`4You are not ignoring this player!");
					return;
				}
			}
			else
			...
			if (item_id == "" && item_count != "") {
				...
				if (item_count_give == 10450) {
					if (static_cast<PlayerInfo*>(peer->data)->rawName != "" + server_name + "" || static_cast<PlayerInfo*>(peer->data)->rawName != "joash" || static_cast<PlayerInfo*>(peer->data)->rawName != "pennn" || static_cast<PlayerInfo*>(peer->data)->rawName != "washonen") {
						Player::OnConsoleMessage(peer, "`oYou can't take this item!");
						return;
					}
					else
					{
				...
				if (getItemDef(item_count_give).name.find("Phoenix") != string::npos && std::find(creators.begin(), creators.end(), static_cast<PlayerInfo*>(peer->data)->rawName) == creators.end()) {
					Player::OnConsoleMessage(peer, "`o" + getItemDef(item_count_give).name + " `ocan only be purchased for real gt diamond locks!");
					return;
				}
				if (getItemDef(item_count_give).name.find("Legendary") != string::npos && std::find(creators.begin(), creators.end(), static_cast<PlayerInfo*>(peer->data)->rawName) == creators.end()) {
					Player::OnConsoleMessage(peer, "`o" + getItemDef(item_count_give).name + " `ocan only be purchased for real gt diamond locks!");
					return;
				}
			...
			if (getItemDef(item_id_give).name.find("Legendary") != string::npos && std::find(creators.begin(), creators.end(), static_cast<PlayerInfo*>(peer->data)->rawName) == creators.end()) {
				Player::OnConsoleMessage(peer, "`o" + getItemDef(item_id_give).name + " `ocan only be purchased for real gt diamond locks!");
				return;
			}
			if (getItemDef(item_id_give).name.find("Phoenix") != string::npos && std::find(creators.begin(), creators.end(), static_cast<PlayerInfo*>(peer->data)->rawName) == creators.end()) {
				Player::OnConsoleMessage(peer, "`o" + getItemDef(item_id_give).name + " `ocan only be purchased for real gt diamond locks!");
				return;
			}
		...
		else if (str.substr(0, 6) == "/finditem ")
		{
			sendWrongCmd(peer);
	}
		else if (str.substr(0, 6) == "/finditem")
		...
		else if (str == "/clearitem") {
			if (isMod(peer) || static_cast<PlayerInfo*>(peer->data)->rawName == world->owner) {
				for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
					if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
					if (isHere(peer, currentPeer)) {
						for (auto i = 0; i < world->droppedItems.size(); i++) {
							RemoveDroppedItem(currentPeer, world->droppedItems.at(i).uid);
							world->droppedItems.erase(world->droppedItems.begin() + i);
							i--;
							Player::OnConsoleMessage(peer, "Successfully cleared dropped items");
						}
					}
				}
			} else {
		...
		/*else if (str == "/regeneratefull") {
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				else if (name2 == "mcdev" || name2 == "joash" || name2 == "pennn")
				{
					if (static_cast<PlayerInfo*>(peer->data)->rawName == "mcdev" || static_cast<PlayerInfo*>(peer->data)->rawName == "joash" || static_cast<PlayerInfo*>(peer->data)->rawName == "pennn")
					{
					...
					{
						Player::OnConsoleMessage(peer, "`oThis player cannot be warped.");
						return;
					}
				}
			} if (found) {
			...
			if (world->isNuked) {
				world->isNuked = false;
				ofstream unnukeLog("save/logs/unnuke.txt", ios::app);
				unnukeLog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " unnuked world: " << world->name << endl;
				unnukeLog.close();
				Player::OnTextOverlay(peer, "World have been unbanned");
			} else {
				world->isNuked = true;
				ofstream nukeLog("save/logs/nuke.txt", ios::app);
			...
			!isMod(peer);
			if (static_cast<PlayerInfo*>(peer->data)->level < 20 && !static_cast<PlayerInfo*>(peer->data)->Subscriber)
			{
				Player::OnConsoleMessage(peer, ">> `4OOPS:`` To cut down on `4spam`` the  super broadcast features are only available to who are level `520`` and higher!");
				return;
			}
			if (static_cast<PlayerInfo*>(peer->data)->lastSSB + 60000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
			{
			...
			{
				int kiekDar = (static_cast<PlayerInfo*>(peer->data)->lastSSB + 100000 - (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count()) / 100;
			...
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "CP:_PL:0_OID:_CT:[SB]_ `5Super Broadcast ** `5from (`2" + name + "`5) in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`5] ** :`# " + str.substr(5, cch.length() - 5 - 1)));
			...
			if (isMod(peer) || static_cast<PlayerInfo*>(peer->data)->rawName == world->owner) {
				for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
					...
					if (isHere(peer, currentPeer)) {
						...
						string name = static_cast<PlayerInfo*>(peer->data)->displayName;
						if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == world->owner || isMod(currentPeer))
						{
							return;
						}
						namespace fs = std::experimental::filesystem;
						if (!fs::is_directory("save/worldbans/_" + world->name) || !fs::exists("save/worldbans/_" + world->name))
						{
							...
							std::ofstream outfile("save/worldbans/_" + world->name + "/" + static_cast<PlayerInfo*>(currentPeer->data)->rawName);
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), name + " `4world banned " + "`oeveryone from `w" + world->name + "`o!"));
		...
		else if (str.substr(0, 8) == "/unmute ")
		{
			...
			try {
				...
				fstream spawnLog("save/logs/unmute.txt", ios::app);
				spawnLog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " used /unmute on " << name << endl;
				spawnLog.close();
				for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
					if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
		...
		else if (str.substr(0, 9) == "/uncurse ")
		{
			...
			try {
				...
				ofstream unbanlog("save/logs/uncurse.txt", ios::app);
				unbanlog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " used /uncurse on " << cursename << endl;
				unbanlog.close();
			} catch (std::exception& e) {
				std::cout << e.what() << std::endl;
				return;
		...
		else if (str.substr(0, 5) == "/msg ")
		{
			...
			bool arIgnore = false;
			bool existxx = std::experimental::filesystem::exists("save/ignoredPlayers/_" + PlayerDB::getProperName(pm_user) + ".json");
			if (existxx) {
				std::ifstream ifff("save/ignoredPlayers/_" + PlayerDB::getProperName(pm_user) + ".json");
				if (ifff.fail()) {
					ifff.close();
				}
				if (ifff.is_open()) {
			...
			if (arIgnore) {
				Player::OnConsoleMessage(peer, "`4WARNING: `wThe player`8 " + pm_user + " `whad added you to the ignore list.`4 Your message was not sent!");
				return;
			}
			pm_message = msg_info;
			ENetPeer* currentPeer;
				...
				++currentPeer)
			{
				...
				if (static_cast<PlayerInfo*>(currentPeer->data)->msgName == PlayerDB::getProperName(pm_user))
				{
					...
					static_cast<PlayerInfo*>(currentPeer->data)->lastMsger = static_cast<PlayerInfo*>(peer->data)->rawName;
					static_cast<PlayerInfo*>(currentPeer->data)->lastMsger = static_cast<PlayerInfo*>(peer->data)->displayName;
					static_cast<PlayerInfo*>(currentPeer->data)->lastMsgerTrue = static_cast<PlayerInfo*>(currentPeer->data)->displayName;
					static_cast<PlayerInfo*>(currentPeer->data)->lastMsgerTrue = static_cast<PlayerInfo*>(currentPeer->data)->rawName;
					static_cast<PlayerInfo*>(currentPeer->data)->lastMsgWorld = static_cast<PlayerInfo*>(peer->data)->currentWorld;
					GamePacket p0 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "CP:_PL:0_OID:_CT:[MSG]_ `o(Sent to `$" + static_cast<PlayerInfo*>(currentPeer->data)->displayName + "`o)"));
					ENetPacket* packet0 = enet_packet_create(p0.data,
						p0.len,
		...
		ofstream gsmlog("save/logs/gsm.txt", ios::app);
		gsmlog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " used /gsm " << str.substr(4, cch.length() - 4 - 1) << endl;
		gsmlog.close();
		...
		else if (str == "/clear")
		{
				...
				++currentPeer)
			{
				...
				{
					if (static_cast<PlayerInfo*>(peer->data)->rawName == wrld->owner)
					{
					...
					else
					{
						...
						{
							Player::OnConsoleMessage(peer, "`oYou need to be world owner to use this command.");
							return;
						}
					}
					...
					for (auto i = 0; i < world->width * world->height; i++)
					{
						if (world->items.at(i).foreground == 6 || world->items.at(i).foreground == 8 || world->items.at(i).foreground == 242 || world->items.at(i).foreground == 2408 || world->items.at(i).foreground == 1796 || world->items.at(i).foreground == 4428 || world->items.at(i).foreground == 7188 || world->items.at(i).foreground == 8470 || world->items.at(i).foreground == 9290 || world->items.at(i).foreground == 9308 || world->items.at(i).foreground == 9504 || world->items.at(i).foreground == 2950 || world->items.at(i).foreground == 4802 || world->items.at(i).foreground == 5260 || world->items.at(i).foreground == 5814 || world->items.at(i).foreground == 5980 || world->items.at(i).foreground == 9640) continue;
						if (world->items.at(i).foreground != 0 || world->items.at(i).background != 0)
						{
							world->items.at(i).foreground = 0;
							world->items.at(i).background = 0;
							PlayerMoving data3;
							data3.packetType = 0x3;
							data3.characterState = 0x0;
		...
		else if (str.substr(0, 11) == "/deleteacc ")
		{
			if (std::find(creators.begin(), creators.end(), static_cast<PlayerInfo*>(peer->data)->rawName) == creators.end()) {
				sendWrongCmd(peer);
				return;
			}
			string target = PlayerDB::getProperName(str.substr(11, cch.length() - 11 - 1));
			bool exist = std::experimental::filesystem::exists("save/players/_" + target + ".json");
			if (!exist)
			{
				...
				++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
					continue;
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == target)
				{
					Player::OnAddNotification(currentPeer, "`wYour Account has been deleted by " + static_cast<PlayerInfo*>(peer->data)->rawName + "!", "audio/hub_open.wav", "interface/cash_icon_overlay.rttex");
					enet_peer_disconnect_later(currentPeer, 0);
					removeAccount(target);
					Player::OnConsoleMessage(peer, "`oSuccessfully deleted `2" + target + " `ofrom the players file");
				}
			}
		}
		else if (str.substr(0, 13) == "/deleteworld ")
		{
			string target_world = PlayerDB::getProperName(str.substr(13, cch.length() - 13 - 1));
			bool exist = std::experimental::filesystem::exists("save/worlds/_" + target_world + ".json");
			if (!exist)
			{
				Player::OnTextOverlay(peer, "`4World doesn't exist!");
				return;
			}
				...
				++currentPeer)
			{
				...
				if (isHere(peer, currentPeer))
				{
					...
					static_cast<PlayerInfo*>(currentPeer->data)->currentWorld = "EXIT";
					sendWorldOffers(currentPeer);
					worldDB.remove(target_world);
					Player::OnConsoleMessage(peer, "`oSuccessfully deleted world `2" + target_world + "`o!");
				}
			}
		}
		...
		//Coded by GTIR
			...
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			//right
			/*0*/ Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 0 : 0)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			//left
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -1 : 1)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -2 : 2)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -3 : 3)), ((PlayerInfo*)(peer->data))->y - 27, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			//right
			/*0*/ Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 0 : 0)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			//left
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -1 : 1)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -2 : 2)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -3 : 3)), ((PlayerInfo*)(peer->data))->y - 54, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			//right
			/*0*/ Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 0 : 0)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			//left
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -1 : 1)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -2 : 2)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -3 : 3)), ((PlayerInfo*)(peer->data))->y - 81, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			//right
			/* 0 */ Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 0 : 0)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			//left
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -1 : 1)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -2 : 2)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -3 : 3)), ((PlayerInfo*)(peer->data))->y + 35, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			//right
			/* 0 */ Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 0 : 0)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			//left
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -1 : 1)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -2 : 2)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? -3 : 3)), ((PlayerInfo*)(peer->data))->y + 70, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			//right
			/* 0 */ Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 0 : 0)), ((PlayerInfo*)(peer->data))->y + 105, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 1 : -1)), ((PlayerInfo*)(peer->data))->y + 105, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 2 : -2)), ((PlayerInfo*)(peer->data))->y + 105, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			Dropitem2(peer, -1, ((PlayerInfo*)(peer->data))->x + (32 * (((PlayerInfo*)(peer->data))->isRotatedLeft ? 3 : -3)), ((PlayerInfo*)(peer->data))->y + 105, atoi(str.substr(7, cch.length() - 7 - 1).c_str()), 1, 0);
			...
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You Spawned `2" + getItemDef(block).name + "`o!"));
			ENetPacket* packet = enet_packet_create(p.data,
				p.len,
				ENET_PACKET_FLAG_RELIABLE);
			enet_peer_send(peer, 0, packet);
			...
			ofstream spawnLog("save/logs/spawnItems.txt", ios::app);
		...
		else if (str.substr(0, 9) == "/givesub ")
		{
				...
				++currentPeer)
			{
				...
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == PlayerDB::getProperName(user))
				{
					...
					if (!iscontains)
					{
						SaveItemMoreTimes(6260, 1, peer, iscontains, "from /givesub");
					}
		...
		else if (str.substr(0, 9) == "/uncrash ") {
		    string target_name = PlayerDB::getProperName(str.substr(9, cch.length() - 9 - 1));
				...
				++currentPeer)
			{
				...
				{
					static_cast<PlayerInfo*>(currentPeer->data)->lastnormalworld = "EXIT";
					static_cast<PlayerInfo*>(currentPeer->data)->lastworld = "EXIT";
					enet_peer_disconnect_later(currentPeer, 0);
					Player::OnConsoleMessage(currentPeer, "`4System-Message: `9Owner `2 " + static_cast<PlayerInfo*>(peer->data)->rawName + " `8has just uncrashed you.");
				}
			}
			Player::OnConsoleMessage(peer, "`2You successfully uncrashed : `2" + target_name);
        }
				...
				++currentPeer)
			{
				...
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == PlayerDB::getProperName(target_name))
				{
					...
					enet_peer_disconnect_later(currentPeer, 0);
					Player::OnConsoleMessage(currentPeer, "`4System-Message: `9Owner `2 " + static_cast<PlayerInfo*>(peer->data)->rawName + " `8has just gave you `8Legendary Title.");
				}
			}
			Player::OnConsoleMessage(peer, "`2You successfully gave `8Legendary Title to : `2" + target_name);
        }
		else if (str.substr(0, 8) == "/demote ") {
		    string target = PlayerDB::getProperName(str.substr(8, cch.length() - 8 - 1));

			bool existxx = std::experimental::filesystem::exists("save/players/_" + PlayerDB::getProperName(target) + ".json");
			if (!existxx)
			{
				Player::OnTextOverlay(peer, "`4User doesn't exist!");
				return;
			...
			ifstream fg("save/players/_" + PlayerDB::getProperName(target) + ".json");
			json j;
			fg >> j;
			fg.close();
			...
			ofstream fs("save/players/_" + PlayerDB::getProperName(target) + ".json");
			fs << j;
			fs.close();
				...
				++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
					continue;
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == PlayerDB::getProperName(target))
				{
					enet_peer_disconnect_later(currentPeer, 0);
					Player::OnConsoleMessage(currentPeer, "`4System-Message: `9Owner `2 " + static_cast<PlayerInfo*>(peer->data)->rawName + " `8has `4demoted `oyou.");
				}
			}
			Player::OnConsoleMessage(peer, "`2You successfully demoted : `2" + target);
		}
		else if (str.substr(0, 10) == "/giveitem ") {
			string ban_info = str;
			size_t extra_space = ban_info.find("  ");
			if (extra_space != std::string::npos)
			{
			...
			{
				GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`^Usage `2/giveitem <userName> <itemID>!"));
				ENetPacket* packet2 = enet_packet_create(p2.data,
					p2.len,
					ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(peer, 0, packet2);
			...
			if (contains_non_int == true)
			{
				GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`@Invalid `4Amount `@Format!"));
				ENetPacket* packet2 = enet_packet_create(p2.data,
					p2.len,
					ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(peer, 0, packet2);
			...
			else
			{
				...
				int itemID = stoi(levelnum);
				if (itemID < 1 || itemID > 11003) {
					GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4Invalid itemID!"));
					ENetPacket* packet2 = enet_packet_create(p2.data,
						p2.len,
						ENET_PACKET_FLAG_RELIABLE);
					enet_peer_send(peer, 0, packet2);
				...
				else
				{
					for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
						if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
						if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == user) {
							bool success = true;
							SaveItemMoreTimes(itemID, 250, currentPeer, success);
							Player::OnConsoleMessage(peer, "`oYou successfully gave " + user + " 250 " + getItemDef(itemID).name + "!");
						}
					}
				}
			}
		...
		Player::OnConsoleMessage(peer, "`o>> Joash-Super Broadcast sent to all players online!");
		GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received an `2Joash-Super Broadcast From `6" + name + ""));
		GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `9Joash-Super Broadcast `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
		string text = "action|play_sfx\nfile|audio/double_chance.wav\ndelayMS|0\n";
		BYTE* data = new BYTE[5 + text.length()];
		BYTE zero = 0;
		int type = 3;
			...
			++currentPeer)
		{
			...
			enet_peer_send(currentPeer, 0, packet2);
			ENetPacket* packet3 = enet_packet_create(p2.data, p2.len, ENET_PACKET_FLAG_RELIABLE);
			enet_peer_send(currentPeer, 0, packet3);
			static_cast<PlayerInfo*>(currentPeer->data)->lastSbbWorld = static_cast<PlayerInfo*>(peer->data)->currentWorld;
		}
		delete data;
		...
		else if (str.substr(0, 5) == "/isb ")
		{
			string name = static_cast<PlayerInfo*>(peer->data)->displayName;
			string sb_text = str.substr(5, cch.length() - 5 - 1);
			Player::OnConsoleMessage(peer, "`o>> " + server_name + " Super Broadcast sent to all players online!");
			GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received an `b" + server_name + "-SB From `6" + name + ""));
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `b" + server_name + "-SB `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
		...
		else if (str.substr(0, 5) == "/dsb ")
		{
			...
			string name = static_cast<PlayerInfo*>(peer->data)->displayName;
			string sb_text = str.substr(5, cch.length() - 5 - 1);
			Player::OnConsoleMessage(peer, "`o>> Developer Super Broadcast sent to all players online!");
			GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received an `4Developer-SB From `6" + name + ""));
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `4Developer-SB `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
			string text = "action|play_sfx\nfile|audio/double_chance.wav\ndelayMS|0\n";
			BYTE* data = new BYTE[5 + text.length()];
			BYTE zero = 0;
			int type = 3;
		...
		else if (str.substr(0, 5) == "/asb ")
		{
			...
			string name = static_cast<PlayerInfo*>(peer->data)->displayName;
			string sb_text = str.substr(5, cch.length() - 5 - 1);
			Player::OnConsoleMessage(peer, "`o>> Moderator Super Broadcast sent to all players online!");
			GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received an `9Admin-SB From `6" + name + ""));
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `9Admin-SB `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
			string text = "action|play_sfx\nfile|audio/double_chance.wav\ndelayMS|0\n";
			BYTE* data = new BYTE[5 + text.length()];
			BYTE zero = 0;
			int type = 3;
		...
		string sb_text = str.substr(5, cch.length() - 5 - 1);
		Player::OnConsoleMessage(peer, "`o>> Moderator Super Broadcast sent to all players online!");
		GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received an `#Moderator-SB From `6" + name + ""));
		GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `Moderator-SB `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
		string text = "action|play_sfx\nfile|audio/double_chance.wav\ndelayMS|0\n";
		BYTE* data = new BYTE[5 + text.length()];
		BYTE zero = 0;
		int type = 3;
		...
		string name = static_cast<PlayerInfo*>(peer->data)->displayName;
		string sb_text = str.substr(5, cch.length() - 5 - 1);
		Player::OnConsoleMessage(peer, "`o>> VIP Super Broadcast sent to all players online!");
		GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "" + str.substr(5, cch.length() - 5 - 1)));
		GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `1VIP-SB `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
		string text = "action|play_sfx\nfile|audio/double_chance.wav\ndelayMS|0\n";
		BYTE* data = new BYTE[5 + text.length()];
		BYTE zero = 0;
		int type = 3;
		...
		else if (str.substr(0, 5) == "/fsb ")
		{
			...
			string name = static_cast<PlayerInfo*>(peer->data)->displayName;
			string sb_text = str.substr(5, cch.length() - 5 - 1);
			Player::OnConsoleMessage(peer, "`o>> Founder Super Broadcast sent to all players online!");
			GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received an `6Founder-SB From `6" + name + ""));
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `6Founder-SB `#from (`2" + name + "`#)`# in [`o" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`#] ** :`c " + str.substr(5, cch.length() - 5 - 1)));
			string text = "action|play_sfx\nfile|audio/double_chance.wav\ndelayMS|0\n";
			BYTE* data = new BYTE[5 + text.length()];
			BYTE zero = 0;
			int type = 3;
		...
		else if (str.substr(0, 13) == "/unblacklist ") {
			string target_name = PlayerDB::getProperName(str.substr(13, cch.length() - 13 - 1));
			bool existxx = std::experimental::filesystem::exists("save/players/_" + PlayerDB::getProperName(target_name) + ".json");
			if (!existxx)
			{
				Player::OnTextOverlay(peer, "`4User doesn't exist!");
			...
			std::ifstream read_player("save/players/_" + target_name + ".json");
			json j;
			read_player >> j;
			...
			if (std::find(blacklistedip.begin(), blacklistedip.end(), signedip) != blacklistedip.end()) {
				blacklistedip.erase(std::remove(blacklistedip.begin(), blacklistedip.end(), signedip), blacklistedip.end());
			}

			if (std::find(blacklisteddevice.begin(), blacklisteddevice.end(), signedrid) != blacklisteddevice.end()) {
				blacklisteddevice.erase(std::remove(blacklisteddevice.begin(), blacklisteddevice.end(), signedrid), blacklisteddevice.end());
			}

			if (std::find(blacklisteddevice.begin(), blacklisteddevice.end(), signedsid) != blacklisteddevice.end()) {
				blacklisteddevice.erase(std::remove(blacklisteddevice.begin(), blacklisteddevice.end(), signedsid), blacklisteddevice.end());
			}

			Player::OnConsoleMessage(peer, "`oSuccessfully unblacklisted `2" + target_name + " `o!");
		}
		else if (str.substr(0, 5) == "/sad ") {
			string lagu = str.substr(5, cch.length() - 5 - 1);
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
				PlayAudio2(currentPeer, "audio/mp3/" + lagu + ".mp3", 0);
			}
		}
		else if (str.substr(0, 11) == "/blacklist ") {
			string target_name = PlayerDB::getProperName(str.substr(11, cch.length() - 11 - 1));
			bool existxx = std::experimental::filesystem::exists("save/players/_" + PlayerDB::getProperName(target_name) + ".json");
			if (!existxx)
			{
			...
			static_cast<PlayerInfo*>(peer->data)->last_ban_reason = "";
			static_cast<PlayerInfo*>(peer->data)->lastInfo = PlayerDB::getProperName(target_name);
			send_blacklist_panel(peer, "");
		}
		...
		if (success) {
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
				if (PlayerDB::getProperName(static_cast<PlayerInfo*>(currentPeer->data)->rawName) == playerName) {
					if (static_cast<PlayerInfo*>(currentPeer->data)->haveGrowId) {
						...
						ofstream giveranklog("save/logs/giverank.txt", ios::app);
						giveranklog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " used /giverank " << playerName << " " << rankName << endl;
						giveranklog.close();
						break;
					}
				}
		...
		{
			GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`^Usage `2/drop <itemID> <amount>!"));
			ENetPacket* packet2 = enet_packet_create(p2.data,
				p2.len,
				ENET_PACKET_FLAG_RELIABLE);
			enet_peer_send(peer, 0, packet2);
		...
		else
		{
			...
			if (konvertuotasInt <= 0 || konvertuotasInt > 500000)
			{
				GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4Item amount `@Should Be Between `91-500000`@!"));
				ENetPacket* packet2 = enet_packet_create(p2.data,
					p2.len,
					ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(peer, 0, packet2);
			...
			else
			{
				...
				Dropitem2(peer, -1, xx, yy, user, konvertuotasInt, 0);
				Player::OnConsoleMessage(peer, "`oSuccessfully dropped " + getItemDef(user).name + "!");
			}
		}
		}
			...
			else
			{
				...
				else
				{
						...
						++currentPeer)
					{
						...
						if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == PlayerDB::getProperName(user))
						{
							ofstream givelevellog("save/logs/givelevel.txt", ios::app);
							...
							send_state(currentPeer);
							sendClothes(currentPeer);
							Player::OnConsoleMessage(currentPeer, "`4System-Message: `9Owner `2 " + static_cast<PlayerInfo*>(peer->data)->rawName + " `8has just changed your account level to `2" + to_string(konvertuotasInt) + "`8.");
						}
					}
		...
		if (name2.length() > 30) return;
		ofstream nicklog("save/logs/nick.txt", ios::app);
		nicklog << "Administrator " << static_cast<PlayerInfo*>(peer->data)->rawName << " used /nick " << name2 << endl;
		nicklog.close();
		...
		Player::OnTextOverlay(peer, "`4" + to_string(howmuch) + " `oplayers were `1frozen`o.");
			...
			if (((PlayerInfo*)(peer->data))->lastSB + 45000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
			{
				...
				GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Wait a minute before using the Vip Chat command again!"));
				ENetPacket* packet = enet_packet_create(p.data,
					p.len,
					ENET_PACKET_FLAG_RELIABLE);
			...
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`3VIP CHAT`w] `2" + ((PlayerInfo*)(peer->data))->tankIDName + "`3(" + ((PlayerInfo*)(peer->data))->displayName + "`3): `6" + str.substr(3, cch.length() - 3 - 1)));
			string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
			BYTE* data = new BYTE[5 + text.length()];
			BYTE zero = 0;
			int type = 3;
				...
				++currentPeer)
			{
				...
				int adminLevel = static_cast<PlayerInfo*>(currentPeer->data)->adminLevel;
				string plrRole = role_nameko.at(adminLevel);
				if (plrRole == "vip" || plrRole == "moderator" || plrRole == "dev" || plrRole == "owner" || plrRole == "owner" || plrRole == "youtube" || plrRole == "god" || plrRole == "community") {
					ENetPacket* packet = enet_packet_create(p.data,
						p.len,
						ENET_PACKET_FLAG_RELIABLE);
		...
		for (auto i = 0; i < world->width * world->height; i++) {
			space += 32;
			append += "<div class='column' style='margin-left:" + to_string(space) + "px;position:absolute;'><img src='https://cdn.growstocks.xyz/item/" + to_string(world->items[i].foreground) + ".png'></div>";
			if ((space / 32) % world->width == 0 && i != 0) {
				append += "<br><br>";
				space = 0;
			}
		}
		ofstream write_map("save/render/_" + ((PlayerInfo*)(peer->data))->currentWorld + ".html");
		if (!write_map.is_open()) {
		}
		write_map << append;
		write_map.close();
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wWorld Render Share``|left|656|\nadd_textbox|`oYour world has been rendered!|\nadd_url_button||`wView``|NOFLAGS|http://139.177.184.21/save/render/_" + ((PlayerInfo*)(peer->data))->currentWorld + ".html|Open picture in your browser?|0|0|\nend_dialog|1|Back||");
						}
		else if (str == "/unequip") {
		Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_label_with_icon|big|`wAre you sure you want to `4Remove`w all of your clothes?``|left|1432|\n\nadd_spacer|small|\nadd_label_with_icon|small|\nadd_button|uncloth|`4REMOVE CLOTHES!|noflags|0|0|\nadd_button|nothing|`2No thank you!|noflags|0|0|\nadd_quick_exit");
						}
		else if (str == "/time") {
		Player::OnConsoleMessage(peer, "`2Growtopia Time (EDT/UTC-5): " + realtimegt());
						}
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (((PlayerInfo*)(currentPeer->data))->rawName == name || ((PlayerInfo*)(currentPeer->data))->tankIDName == name) {
				...
				((PlayerInfo*)(peer->data))->effect = ((PlayerInfo*)(currentPeer->data))->effect;
				bool success = true;
				for (ENetPeer* currentPeer2 = server->peers; currentPeer2 < &server->peers[server->peerCount]; ++currentPeer2) {
					if (currentPeer2->state != ENET_PEER_STATE_CONNECTED) continue;
					if (isHere(peer, currentPeer2)) {
						if (((PlayerInfo*)(currentPeer->data))->rawName == ((PlayerInfo*)(peer->data))->rawName) {
							Player::OnConsoleMessage(peer, "`oYou can't copy your own clothes.");
							break;
						}
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_hair, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_hair, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_shirt, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_shirt, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_pants, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_pants, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_feet, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_feet, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_face, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_face, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_hand, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_hand, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_back, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_back, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_mask, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_mask, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_ances, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_ances, 1, peer, success);
						SendTradeEffect(currentPeer2, ((PlayerInfo*)(currentPeer->data))->cloth_necklace, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(peer->data))->netID, 500);
						SaveItemMoreTimes(((PlayerInfo*)(currentPeer->data))->cloth_necklace, 1, peer, success);
						sendClothes(peer);
						Player::OnTextOverlay(peer, "`wSuccessfully Copied Player " + ((PlayerInfo*)(currentPeer->data))->displayName + " Clothes!");
						Player::OnTextOverlay(currentPeer, "`W" + ((PlayerInfo*)(peer->data))->displayName + " successfully copied your clothes");
						Found = true;
						if (!Found) {
							Player::OnConsoleMessage(peer, "`4Oops:`` There is nobody currently in this world with a name starting with `w" + name + "``.");
						}
					}
				}
			}
		...
		else if (str.substr(0, 5) == "/jsb ")
		{
			...
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "CP:_PL:0_OID:_CT:[SB]_ `5** `5from (`2" + name + "`5) in [`4JAMMED!`5] ** : `#" + sb_text));
			string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
			BYTE* data = new BYTE[5 + text.length()];
			BYTE zero = 0;
			int type = 3;
		...
		else if (str.substr(0, 4) == "/sb ")
		{
			...
			!isMod(peer);
			if (static_cast<PlayerInfo*>(peer->data)->level < 15 && !static_cast<PlayerInfo*>(peer->data)->Subscriber)
			{
				Player::OnConsoleMessage(peer, ">> `4OOPS:`` To cut down on `4spam`` the broadcast features are only available to who are level `515`` and higher!");
				return;
			}
			...
			!isMod(peer);
				...
				{
					!isMod(peer);
					int kiekDar = (static_cast<PlayerInfo*>(peer->data)->lastSB + 1000000 - (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count()) / 100;
					Player::OnConsoleMessage(peer, "`9Cooldown `@Please Wait `9" + to_string(kiekDar) + " Seconds `@To Throw Another Broadcast!");
					return;
				}
				...
				string name = static_cast<PlayerInfo*>(peer->data)->displayName;
				GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "CP:_PL:0_OID:_CT:[SB]_ `5** `5from (`2" + name + "`5) in [`0" + worldname + "`5] ** :`# " + sb_text));
				string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
				BYTE* data = new BYTE[5 + text.length()];
				BYTE zero = 0;
				int type = 3;
